...
import type { Contract, ContractDetails, ContractDescription, DeltaNeutralContract } from './contract.js';
...
import type { BarData, HistogramData, NewsProvider, DepthMktDataDescription, SmartComponent, FamilyCode, PriceIncrement, HistoricalTick, HistoricalTickBidAsk, HistoricalTickLast, HistoricalSession, TickAttrib, TickAttribBidAsk, TickAttribLast } from './common.js';
...
export interface EWrapper {
    ...
    tickPrice(reqId: number, tickType: number, price: number, attrib: TickAttrib): void;
    ...
    tickEFP(reqId: number, tickType: number, basisPoints: number, formattedBasisPoints: string, totalDividends: number, holdDays: number, futureLastTradeDate: string, dividendImpact: number, dividendsToLastTradeDate: number): void;
    tickOptionComputation(reqId: number, tickType: number, tickAttrib: number, impliedVol: number | null, delta: number | null, optPrice: number | null, pvDividend: number | null, gamma: number | null, vega: number | null, theta: number | null, undPrice: number | null): void;
    tickReqParams(tickerId: number, minTick: number, bboExchange: string, snapshotPermissions: number): void;
    tickNews(tickerId: number, timeStamp: number, providerCode: string, articleId: string, headline: string, extraData: string): void;
    orderStatus(orderId: number, status: string, filled: Decimal, remaining: Decimal, avgFillPrice: number, permId: number, parentId: number, lastFillPrice: number, clientId: number, whyHeld: string, mktCapPrice: number): void;
    ...
    updateAccountValue(key: string, val: string, currency: string, accountName: string): void;
    updatePortfolio(contract: Contract, position: Decimal, marketPrice: string, marketValue: string, averageCost: string, unrealizedPNL: string, realizedPNL: string, accountName: string): void;
    ...
    accountUpdateMulti(reqId: number, account: string, modelCode: string, key: string, value: string, currency: string): void;
    accountUpdateMultiEnd(reqId: number): void;
    position(account: string, contract: Contract, position: Decimal, avgCost: number): void;
    positionEnd(): void;
    positionMulti(reqId: number, account: string, modelCode: string, contract: Contract, pos: Decimal, avgCost: number): void;
    ...
    commissionAndFeesReport(commissionAndFeesReport: CommissionAndFeesReport): void;
    updateMktDepth(reqId: number, position: number, operation: number, side: number, price: number, size: Decimal): void;
    updateMktDepthL2(reqId: number, position: number, marketMaker: string, operation: number, side: number, price: number, size: Decimal, isSmartDepth: boolean): void;
    mktDepthExchanges(depthMktDataDescriptions: DepthMktDataDescription[]): void;
    updateNewsBulletin(msgId: number, msgType: number, newsMessage: string, originExch: string): void;
    newsProviders(newsProviders: NewsProvider[]): void;
    newsArticle(requestId: number, articleType: number, articleText: string): void;
    historicalNews(requestId: number, time: string, providerCode: string, articleId: string, headline: string): void;
    ...
    historicalSchedule(reqId: number, startDateTime: string, endDateTime: string, timeZone: string, sessions: HistoricalSession[]): void;
    headTimestamp(reqId: number, headTimestamp: string): void;
    histogramData(reqId: number, items: HistogramData[]): void;
    historicalTicks(reqId: number, ticks: HistoricalTick[], done: boolean): void;
    historicalTicksBidAsk(reqId: number, ticks: HistoricalTickBidAsk[], done: boolean): void;
    historicalTicksLast(reqId: number, ticks: HistoricalTickLast[], done: boolean): void;
    scannerParameters(xml: string): void;
    scannerData(reqId: number, rank: number, contractDetails: ContractDetails, distance: string, benchmark: string, projection: string, legsStr: string): void;
    scannerDataEnd(reqId: number): void;
    realtimeBar(reqId: number, time: number, open_: number, high: number, low: number, close: number, volume: Decimal, wap: Decimal, count: number): void;
    tickByTickAllLast(reqId: number, tickType: number, time: number, price: number, size: Decimal, tickAttribLast: TickAttribLast, exchange: string, specialConditions: string): void;
    tickByTickBidAsk(reqId: number, time: number, bidPrice: number, askPrice: number, bidSize: Decimal, askSize: Decimal, tickAttribBidAsk: TickAttribBidAsk): void;
    ...
    deltaNeutralValidation(reqId: number, deltaNeutralContract: DeltaNeutralContract): void;
    ...
    smartComponents(reqId: number, smartComponentMap: SmartComponent[]): void;
    marketRule(marketRuleId: number, priceIncrements: PriceIncrement[]): void;
    pnl(reqId: number, dailyPnL: number, unrealizedPnL: number | null, realizedPnL: number | null): void;
    pnlSingle(reqId: number, pos: Decimal, dailyPnL: number, unrealizedPnL: number | null, realizedPnL: number | null, value: number): void;
...
export declare class DefaultEWrapper implements EWrapper {
    error(_reqId: number, _errorTime: number, _errorCode: number, _errorString: string, _advancedOrderRejectJson?: string): void;
    winError(_text: string, _lastError: number): void;
    connectAck(): void;
    connectionClosed(): void;
    marketDataType(_reqId: number, _marketDataType: number): void;
    tickPrice(_reqId: number, _tickType: number, _price: number, _attrib: TickAttrib): void;
    ...
    tickEFP(_reqId: number, _tickType: number, _basisPoints: number, _formattedBasisPoints: string, _totalDividends: number, _holdDays: number, _futureLastTradeDate: string, _dividendImpact: number, _dividendsToLastTradeDate: number): void;
    tickOptionComputation(_reqId: number, _tickType: number, _tickAttrib: number, _impliedVol: number | null, _delta: number | null, _optPrice: number | null, _pvDividend: number | null, _gamma: number | null, _vega: number | null, _theta: number | null, _undPrice: number | null): void;
    tickReqParams(_tickerId: number, _minTick: number, _bboExchange: string, _snapshotPermissions: number): void;
    tickNews(_tickerId: number, _timeStamp: number, _providerCode: string, _articleId: string, _headline: string, _extraData: string): void;
    orderStatus(_orderId: number, _status: string, _filled: Decimal, _remaining: Decimal, _avgFillPrice: number, _permId: number, _parentId: number, _lastFillPrice: number, _clientId: number, _whyHeld: string, _mktCapPrice: number): void;
    openOrder(_orderId: number, _contract: Contract, _order: Order, _orderState: OrderState): void;
    openOrderEnd(): void;
    completedOrder(_contract: Contract, _order: Order, _orderState: OrderState): void;
    completedOrdersEnd(): void;
    orderBound(_permId: number, _clientId: number, _orderId: number): void;
    updateAccountValue(_key: string, _val: string, _currency: string, _accountName: string): void;
    updatePortfolio(_contract: Contract, _position: Decimal, _marketPrice: string, _marketValue: string, _averageCost: string, _unrealizedPNL: string, _realizedPNL: string, _accountName: string): void;
    ...
    accountSummary(_reqId: number, _account: string, _tag: string, _value: string, _currency: string): void;
    accountSummaryEnd(_reqId: number): void;
    accountUpdateMulti(_reqId: number, _account: string, _modelCode: string, _key: string, _value: string, _currency: string): void;
    accountUpdateMultiEnd(_reqId: number): void;
    position(_account: string, _contract: Contract, _position: Decimal, _avgCost: number): void;
    positionEnd(): void;
    positionMulti(_reqId: number, _account: string, _modelCode: string, _contract: Contract, _pos: Decimal, _avgCost: number): void;
    ...
    bondContractDetails(_reqId: number, _contractDetails: ContractDetails): void;
    contractDetailsEnd(_reqId: number): void;
    execDetails(_reqId: number, _contract: Contract, _execution: Execution): void;
    execDetailsEnd(_reqId: number): void;
    commissionAndFeesReport(_commissionAndFeesReport: CommissionAndFeesReport): void;
    updateMktDepth(_reqId: number, _position: number, _operation: number, _side: number, _price: number, _size: Decimal): void;
    updateMktDepthL2(_reqId: number, _position: number, _marketMaker: string, _operation: number, _side: number, _price: number, _size: Decimal, _isSmartDepth: boolean): void;
    mktDepthExchanges(_depthMktDataDescriptions: DepthMktDataDescription[]): void;
    updateNewsBulletin(_msgId: number, _msgType: number, _newsMessage: string, _originExch: string): void;
    newsProviders(_newsProviders: NewsProvider[]): void;
    newsArticle(_requestId: number, _articleType: number, _articleText: string): void;
    historicalNews(_requestId: number, _time: string, _providerCode: string, _articleId: string, _headline: string): void;
    ...
    historicalSchedule(_reqId: number, _startDateTime: string, _endDateTime: string, _timeZone: string, _sessions: HistoricalSession[]): void;
    headTimestamp(_reqId: number, _headTimestamp: string): void;
    histogramData(_reqId: number, _items: HistogramData[]): void;
    historicalTicks(_reqId: number, _ticks: HistoricalTick[], _done: boolean): void;
    historicalTicksBidAsk(_reqId: number, _ticks: HistoricalTickBidAsk[], _done: boolean): void;
    historicalTicksLast(_reqId: number, _ticks: HistoricalTickLast[], _done: boolean): void;
    scannerParameters(_xml: string): void;
    scannerData(_reqId: number, _rank: number, _contractDetails: ContractDetails, _distance: string, _benchmark: string, _projection: string, _legsStr: string): void;
    scannerDataEnd(_reqId: number): void;
    realtimeBar(_reqId: number, _time: number, _open_: number, _high: number, _low: number, _close: number, _volume: Decimal, _wap: Decimal, _count: number): void;
    tickByTickAllLast(_reqId: number, _tickType: number, _time: number, _price: number, _size: Decimal, _tickAttribLast: TickAttribLast, _exchange: string, _specialConditions: string): void;
    tickByTickBidAsk(_reqId: number, _time: number, _bidPrice: number, _askPrice: number, _bidSize: Decimal, _askSize: Decimal, _tickAttribBidAsk: TickAttribBidAsk): void;
    tickByTickMidPoint(_reqId: number, _time: number, _midPoint: number): void;
    ...
    deltaNeutralValidation(_reqId: number, _deltaNeutralContract: DeltaNeutralContract): void;
    securityDefinitionOptionParameter(_reqId: number, _exchange: string, _underlyingConId: number, _tradingClass: string, _multiplier: string, _expirations: Set<string>, _strikes: Set<number>): void;
    ...
    symbolSamples(_reqId: number, _contractDescriptions: ContractDescription[]): void;
    smartComponents(_reqId: number, _smartComponentMap: SmartComponent[]): void;
    marketRule(_marketRuleId: number, _priceIncrements: PriceIncrement[]): void;
    pnl(_reqId: number, _dailyPnL: number, _unrealizedPnL: number | null, _realizedPnL: number | null): void;
    pnlSingle(_reqId: number, _pos: Decimal, _dailyPnL: number, _unrealizedPnL: number | null, _realizedPnL: number | null, _value: number): void;
    rerouteMktDataReq(_reqId: number, _conId: number, _exchange: string): void;
    rerouteMktDepthReq(_reqId: number, _conId: number, _exchange: string): void;
