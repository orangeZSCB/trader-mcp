...
import { decodeStr, decodeInt, decodeFloat, decodeBool, } from '../utils.js';
import { MIN_SERVER_VER_ADVANCED_ORDER_REJECT, MIN_SERVER_VER_ERROR_TIME, MIN_SERVER_VER_SERVICE_DATA_TYPE, } from '../server-versions.js';
import { SoftDollarTier } from '../softdollartier.js';
import { FamilyCode, SmartComponent, DepthMktDataDescription, NewsProvider, } from '../common.js';
...
function decodeSoftDollarTierFromProto(p) {
    return new SoftDollarTier(p.name ?? '', p.value ?? '', p.displayName ?? '');
}
function decodeFamilyCodeFromProto(p) {
    const fc = new FamilyCode();
    fc.accountID = p.accountId ?? '';
    fc.familyCodeStr = p.familyCode ?? '';
    return fc;
}
function decodeSmartComponentFromProto(p) {
...
function decodeDepthMktDataDescFromProto(p) {
...
// Registration
// ---------------------------------------------------------------------------
export function applyMiscHandlers(decoder) {
    // ========================================================================
    // Text handlers
    // ========================================================================
    // --- IN.ERR_MSG (4) ---
    ...
    decoder.registerText(IN.SCANNER_DATA, (d, fields) => {
        ...
        for (let i = 0; i < numberOfElements; i++) {
            ...
            d.wrapper.scannerData(reqId, data.rank, contractDetails, data.distance, data.benchmark, data.projection, data.legsStr);
        }
    ...
    decoder.registerProto(IN.ERR_MSG, (d, buf) => {
        ...
        d.wrapper.error(proto.id ?? -1, proto.errorTime ?? 0, proto.errorCode ?? 0, proto.errorMsg ?? '', proto.advancedOrderRejectJson ?? '');
    });
    // --- IN.CURRENT_TIME (49) ---
    ...
    decoder.registerProto(IN.NEWS_BULLETINS, (d, buf) => {
        ...
        d.wrapper.updateNewsBulletin(proto.newsMsgId ?? 0, proto.newsMsgType ?? 0, proto.newsMessage ?? '', proto.originatingExch ?? '');
    });
    // --- IN.RECEIVE_FA (16) ---
    decoder.registerProto(IN.RECEIVE_FA, (d, buf) => {
        const proto = ReceiveFAProto.decode(buf);
        d.wrapper.receiveFA(proto.faDataType ?? 0, proto.xml ?? '');
    });
    // --- IN.SCANNER_PARAMETERS (19) ---
    ...
    decoder.registerProto(IN.FUNDAMENTAL_DATA, (d, buf) => {
        ...
        d.wrapper.fundamentalData(proto.reqId ?? NO_VALID_ID, proto.data ?? '');
    });
    // --- IN.NEWS_PROVIDERS (85) ---
    ...
    decoder.registerProto(IN.NEWS_ARTICLE, (d, buf) => {
        ...
        d.wrapper.newsArticle(proto.reqId ?? NO_VALID_ID, proto.articleType ?? 0, proto.articleText ?? '');
    });
    // --- IN.TICK_NEWS (84) ---
    decoder.registerProto(IN.TICK_NEWS, (d, buf) => {
        const proto = TickNewsProto.decode(buf);
        d.wrapper.tickNews(proto.reqId ?? NO_VALID_ID, proto.timestamp ?? 0, proto.providerCode ?? '', proto.articleId ?? '', proto.headline ?? '', proto.extraData ?? '');
    });
    // --- IN.HISTORICAL_NEWS (86) ---
    decoder.registerProto(IN.HISTORICAL_NEWS, (d, buf) => {
        const proto = HistoricalNewsProto.decode(buf);
        d.wrapper.historicalNews(proto.reqId ?? NO_VALID_ID, proto.time ?? '', proto.providerCode ?? '', proto.articleId ?? '', proto.headline ?? '');
    });
    // --- IN.HISTORICAL_NEWS_END (87) ---
    decoder.registerProto(IN.HISTORICAL_NEWS_END, (d, buf) => {
        const proto = HistoricalNewsEndProto.decode(buf);
        d.wrapper.historicalNewsEnd(proto.reqId ?? NO_VALID_ID, proto.hasMore ?? false);
    });
    // --- IN.SECURITY_DEFINITION_OPTION_PARAMETER (75) ---
