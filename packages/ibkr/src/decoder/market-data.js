...
import { MIN_SERVER_VER_PAST_LIMIT, MIN_SERVER_VER_PRE_OPEN_BID_ASK, MIN_SERVER_VER_SMART_DEPTH, MIN_SERVER_VER_PRICE_BASED_VOLATILITY, } from '../server-versions.js';
...
export function applyMarketDataHandlers(decoder) {
    // ----------------------------------------------------------------
    // Text handlers
    // ----------------------------------------------------------------
    // IN.TICK_PRICE (1)
    decoder.registerText(IN.TICK_PRICE, (d, fields) => {
        ...
        let sizeTickType = TickTypeEnum.NOT_SET;
    ...
    decoder.registerText(IN.TICK_OPTION_COMPUTATION, (d, fields) => {
        ...
        let impliedVol = decodeFloat(fields);
        let delta = decodeFloat(fields);
        if (impliedVol < 0)
            impliedVol = null; // -1 = not computed
        if (delta === -2)
            delta = null; // -2 = not computed
        if (version >= 6 ||
        ...
        d.wrapper.tickOptionComputation(reqId, tickTypeInt, tickAttrib, impliedVol, delta, optPrice, pvDividend, gamma, vega, theta, undPrice);
    });
    // IN.TICK_GENERIC (45)
    ...
    decoder.registerText(IN.TICK_EFP, (d, fields) => {
        ...
        d.wrapper.tickEFP(reqId, tickType, basisPoints, formattedBasisPoints, impliedFuturesPrice, holdDays, futureLastTradeDate, dividendImpact, dividendsToLastTradeDate);
    });
    ...
    decoder.registerProto(IN.TICK_OPTION_COMPUTATION, (d, buf) => {
        ...
        let impliedVol = proto.impliedVol ?? null;
        if (impliedVol !== null && impliedVol < 0)
            impliedVol = null; // -1 = not computed
        let delta = proto.delta ?? null;
        if (delta !== null && delta === -2)
            delta = null; // -2 = not computed
        let optPrice = proto.optPrice ?? null;
        if (optPrice !== null && optPrice === -1)
            optPrice = null; // -1 = not computed
        let pvDividend = proto.pvDividend ?? null;
        if (pvDividend !== null && pvDividend === -1)
            pvDividend = null; // -1 = not computed
        let gamma = proto.gamma ?? null;
        if (gamma !== null && gamma === -2)
            gamma = null; // -2 = not yet computed
        let vega = proto.vega ?? null;
        if (vega !== null && vega === -2)
            vega = null; // -2 = not yet computed
        let theta = proto.theta ?? null;
        if (theta !== null && theta === -2)
            theta = null; // -2 = not yet computed
        let undPrice = proto.undPrice ?? null;
        if (undPrice !== null && undPrice === -1)
            undPrice = null; // -1 = not computed
    ...
    decoder.registerProto(IN.MARKET_DEPTH, (d, buf) => {
        ...
        if (proto.marketDepthData === undefined)
            return;
