...
import { decodeStr, decodeInt, decodeFloat, decodeDecimal, floatMaxString, } from '../utils.js';
import { MIN_SERVER_VER_UNREALIZED_PNL, MIN_SERVER_VER_REALIZED_PNL, } from '../server-versions.js';
...
// ---------------------------------------------------------------------------
function decodeContractProto(cp) {
    ...
    if (cp.secType !== undefined)
        c.secType = coerceSecType(cp.secType);
    if (cp.lastTradeDateOrContractMonth !== undefined)
        c.lastTradeDateOrContractMonth = cp.lastTradeDateOrContractMonth;
    if (cp.strike !== undefined)
        c.strike = cp.strike;
    if (cp.right !== undefined)
        c.right = cp.right;
    if (cp.multiplier !== undefined)
        c.multiplier = floatMaxString(cp.multiplier);
    if (cp.exchange !== undefined)
        c.exchange = cp.exchange;
    if (cp.currency !== undefined)
        c.currency = cp.currency;
    if (cp.localSymbol !== undefined)
        c.localSymbol = cp.localSymbol;
    if (cp.tradingClass !== undefined)
        c.tradingClass = cp.tradingClass;
    if (cp.comboLegsDescrip !== undefined)
        c.comboLegsDescrip = cp.comboLegsDescrip;
    if (cp.lastTradeDate !== undefined)
        c.lastTradeDate = cp.lastTradeDate;
    if (cp.primaryExch !== undefined)
        c.primaryExchange = cp.primaryExch;
    if (cp.issuerId !== undefined)
        c.issuerId = cp.issuerId;
...
// Registration
// ---------------------------------------------------------------------------
export function applyAccountHandlers(decoder) {
    // ===========================================================================
    // IN.ACCT_VALUE (6)
    // ===========================================================================
