...
import { MIN_SERVER_VER_SYNT_REALTIME_BARS, MIN_SERVER_VER_HISTORICAL_DATA_END, } from '../server-versions.js';
import { BarData, RealTimeBar, HistogramData, HistoricalTick, HistoricalTickBidAsk, HistoricalTickLast, HistoricalSession, TickAttribBidAsk, TickAttribLast, } from '../common.js';
...
// ----------------------------------------------------------------
function decodeHistoricalDataBar(proto) {
    const bar = new BarData();
    if (proto.date !== undefined)
        bar.date = proto.date;
    if (proto.open !== undefined)
        bar.open = proto.open;
    if (proto.high !== undefined)
        bar.high = proto.high;
    if (proto.low !== undefined)
        bar.low = proto.low;
    if (proto.close !== undefined)
        bar.close = proto.close;
    if (proto.volume !== undefined)
        bar.volume = new Decimal(proto.volume);
    if (proto.WAP !== undefined)
        bar.wap = new Decimal(proto.WAP);
    if (proto.barCount !== undefined)
        bar.barCount = proto.barCount;
    return bar;
}
function decodeHistogramDataEntry(proto) {
    const hd = new HistogramData();
    if (proto.price !== undefined)
        hd.price = proto.price;
    if (proto.size !== undefined)
        hd.size = new Decimal(proto.size);
    return hd;
}
function decodeHistoricalTickProto(proto) {
    const tick = new HistoricalTick();
    if (proto.time !== undefined)
        tick.time = proto.time;
    if (proto.price !== undefined)
        tick.price = proto.price;
    if (proto.size !== undefined)
        tick.size = new Decimal(proto.size);
    return tick;
}
function decodeHistoricalTickBidAskProto(proto) {
    const tick = new HistoricalTickBidAsk();
    if (proto.time !== undefined)
        tick.time = proto.time;
    const attr = new TickAttribBidAsk();
    if (proto.tickAttribBidAsk !== undefined) {
        if (proto.tickAttribBidAsk.bidPastLow !== undefined)
            attr.bidPastLow = proto.tickAttribBidAsk.bidPastLow;
        if (proto.tickAttribBidAsk.askPastHigh !== undefined)
            attr.askPastHigh = proto.tickAttribBidAsk.askPastHigh;
    }
    tick.tickAttribBidAsk = attr;
    if (proto.priceBid !== undefined)
        tick.priceBid = proto.priceBid;
    if (proto.priceAsk !== undefined)
        tick.priceAsk = proto.priceAsk;
    if (proto.sizeBid !== undefined)
        tick.sizeBid = new Decimal(proto.sizeBid);
    if (proto.sizeAsk !== undefined)
        tick.sizeAsk = new Decimal(proto.sizeAsk);
    return tick;
}
function decodeHistoricalTickLastProto(proto) {
    const tick = new HistoricalTickLast();
    if (proto.time !== undefined)
        tick.time = proto.time;
    const attr = new TickAttribLast();
    if (proto.tickAttribLast !== undefined) {
        if (proto.tickAttribLast.pastLimit !== undefined)
            attr.pastLimit = proto.tickAttribLast.pastLimit;
        if (proto.tickAttribLast.unreported !== undefined)
            attr.unreported = proto.tickAttribLast.unreported;
    }
    ...
    if (proto.specialConditions !== undefined)
        tick.specialConditions = proto.specialConditions;
    return tick;
}
// ----------------------------------------------------------------
// Text handlers
// ----------------------------------------------------------------
function processHistoricalDataMsg(d, fields) {
...
function processHistoricalDataEndMsg(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
...
function processHistoricalDataUpdateMsg(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
    const bar = new BarData();
...
function processRealTimeBarMsg(d, fields) {
    decodeInt(fields); // msgId
    decodeInt(fields); // version
    const reqId = decodeInt(fields);
    ...
    d.wrapper.realtimeBar(reqId, bar.time, bar.open_, bar.high, bar.low, bar.close, bar.volume, bar.wap, bar.count);
}
function processHeadTimestamp(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
    const headTimestamp = decodeStr(fields);
    d.wrapper.headTimestamp(reqId, headTimestamp);
}
function processHistogramData(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
...
function processHistoricalTicks(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
...
function processHistoricalTicksBidAsk(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
    const tickCount = decodeInt(fields);
    const ticks = [];
...
function processHistoricalTicksLast(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
    const tickCount = decodeInt(fields);
    const ticks = [];
...
function processTickByTickMsg(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
...
function processHistoricalSchedule(d, fields) {
    decodeInt(fields); // msgId
    const reqId = decodeInt(fields);
...
// Protobuf handlers
// ----------------------------------------------------------------
function processHistoricalDataMsgProtoBuf(d, buf) {
    const proto = HistoricalDataProto.decode(buf);
    const reqId = proto.reqId ?? NO_VALID_ID;
    if (!proto.historicalDataBars || proto.historicalDataBars.length === 0)
        return;
...
function processHistoricalDataEndMsgProtoBuf(d, buf) {
...
function processHistoricalDataUpdateMsgProtoBuf(d, buf) {
    const proto = HistoricalDataUpdateProto.decode(buf);
    const reqId = proto.reqId ?? NO_VALID_ID;
    if (proto.historicalDataBar === undefined)
        return;
    const bar = decodeHistoricalDataBar(proto.historicalDataBar);
    d.wrapper.historicalDataUpdate(reqId, bar);
}
function processRealTimeBarMsgProtoBuf(d, buf) {
...
function processHeadTimestampMsgProtoBuf(d, buf) {
    const proto = HeadTimestampProto.decode(buf);
    const reqId = proto.reqId ?? NO_VALID_ID;
    const headTimestamp = proto.headTimestamp ?? '';
    d.wrapper.headTimestamp(reqId, headTimestamp);
}
function processHistogramDataMsgProtoBuf(d, buf) {
...
function processHistoricalTicksMsgProtoBuf(d, buf) {
...
function processHistoricalTicksBidAskMsgProtoBuf(d, buf) {
...
function processHistoricalTicksLastMsgProtoBuf(d, buf) {
...
function processTickByTickMsgProtoBuf(d, buf) {
    ...
    if (tickType === 0) {
        ...
        if (proto.historicalTickLast !== undefined) {
            ...
            d.wrapper.tickByTickAllLast(reqId, tickType, tick.time, tick.price, tick.size, tick.tickAttribLast, tick.exchange, tick.specialConditions);
        }
    }
    else if (tickType === 3) {
        // BidAsk
        if (proto.historicalTickBidAsk !== undefined) {
            ...
            d.wrapper.tickByTickBidAsk(reqId, tick.time, tick.priceBid, tick.priceAsk, tick.sizeBid, tick.sizeAsk, tick.tickAttribBidAsk);
        }
    }
    else if (tickType === 4) {
        // MidPoint
...
function processHistoricalScheduleMsgProtoBuf(d, buf) {
...
// Registration
// ----------------------------------------------------------------
export function applyHistoricalHandlers(decoder) {
    // Text handlers
