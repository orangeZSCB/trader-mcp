...
import { MIN_SERVER_VER_LAST_LIQUIDITY, MIN_SERVER_VER_MODELS_SUPPORT, MIN_SERVER_VER_PENDING_PRICE_REVISION, MIN_SERVER_VER_SUBMITTER, } from '../server-versions.js';
...
function decodeContractFromProto(cp) {
    ...
    if (cp.multiplier !== undefined)
        contract.multiplier = String(cp.multiplier);
    ...
    if (cp.deltaNeutralContract !== undefined) {
        ...
        if (cp.deltaNeutralContract.conId !== undefined)
            dnc.conId = cp.deltaNeutralContract.conId;
        if (cp.deltaNeutralContract.delta !== undefined)
            dnc.delta = cp.deltaNeutralContract.delta;
        if (cp.deltaNeutralContract.price !== undefined)
            dnc.price = cp.deltaNeutralContract.price;
...
function decodeExecutionFromProto(ep) {
    const execution = new Execution();
    if (ep.orderId !== undefined)
        execution.orderId = ep.orderId;
    if (ep.clientId !== undefined)
        execution.clientId = ep.clientId;
    if (ep.execId !== undefined)
        execution.execId = ep.execId;
    if (ep.time !== undefined)
        execution.time = ep.time;
    if (ep.acctNumber !== undefined)
        execution.acctNumber = ep.acctNumber;
    if (ep.exchange !== undefined)
        execution.exchange = ep.exchange;
    if (ep.side !== undefined)
        execution.side = ep.side;
    if (ep.shares !== undefined)
        execution.shares = new Decimal(ep.shares);
    if (ep.price !== undefined)
        execution.price = ep.price;
    if (ep.permId !== undefined)
        execution.permId = ep.permId;
    if (ep.isLiquidation !== undefined)
        execution.liquidation = ep.isLiquidation ? 1 : 0;
    if (ep.cumQty !== undefined)
        execution.cumQty = new Decimal(ep.cumQty);
    if (ep.avgPrice !== undefined)
        execution.avgPrice = ep.avgPrice;
    if (ep.orderRef !== undefined)
        execution.orderRef = ep.orderRef;
    if (ep.evRule !== undefined)
        execution.evRule = ep.evRule;
    if (ep.evMultiplier !== undefined)
        execution.evMultiplier = ep.evMultiplier;
    if (ep.modelCode !== undefined)
        execution.modelCode = ep.modelCode;
    if (ep.lastLiquidity !== undefined)
        execution.lastLiquidity = ep.lastLiquidity;
    if (ep.isPriceRevisionPending !== undefined)
        execution.pendingPriceRevision = ep.isPriceRevisionPending;
    if (ep.submitter !== undefined)
        execution.submitter = ep.submitter;
    if (ep.optExerciseOrLapseType !== undefined) {
        ...
        if (entry)
            execution.optExerciseOrLapseType = entry;
    }
    return execution;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
export function applyExecutionHandlers(decoder) {
    // ── Text handlers ──────────────────────────────────────────────────────
    // IN.EXECUTION_DATA (11)
