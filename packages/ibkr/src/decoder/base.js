...
export class Decoder {
    ...
    interpret(fields, msgId) {
        ...
        try {
            ...
            if (e instanceof BadMessage) {
                this.wrapper.error(NO_VALID_ID, currentTimeMillis(), BAD_MESSAGE.code(), BAD_MESSAGE.msg() + fields.join(','), '');
            }
            throw e;
        }
    }
     ...
     */
    processProtoBuf(protoBuf, msgId) {
        if (msgId === 0)
            return;
        ...
        try {
            ...
            if (e instanceof BadMessage) {
                this.wrapper.error(NO_VALID_ID, currentTimeMillis(), BAD_MESSAGE.code(), BAD_MESSAGE.msg() + `protobuf msgId=${msgId}`, '');
            }
            throw e;
        }
    }
