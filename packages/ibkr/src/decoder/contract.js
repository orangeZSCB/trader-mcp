...
import { Contract, ContractDetails, ContractDescription, DeltaNeutralContract, ComboLeg, FundDistributionPolicyIndicator, FundAssetType, coerceSecType, } from '../contract.js';
...
import { MIN_SERVER_VER_SIZE_RULES, MIN_SERVER_VER_MD_SIZE_MULTIPLIER, MIN_SERVER_VER_ENCODE_MSG_ASCII7, MIN_SERVER_VER_AGG_GROUP, MIN_SERVER_VER_UNDERLYING_INFO, MIN_SERVER_VER_MARKET_RULES, MIN_SERVER_VER_REAL_EXPIRATION_DATE, MIN_SERVER_VER_STOCK_TYPE, MIN_SERVER_VER_FRACTIONAL_SIZE_SUPPORT, MIN_SERVER_VER_FUND_DATA_FIELDS, MIN_SERVER_VER_INELIGIBILITY_REASONS, MIN_SERVER_VER_LAST_TRADE_DATE, MIN_SERVER_VER_BOND_TRADING_HOURS, MIN_SERVER_VER_BOND_ISSUERID, } from '../server-versions.js';
...
// Helpers
// ---------------------------------------------------------------------------
function getEnumTypeFromString(enumObj, code) {
...
function setLastTradeDate(lastTradeDateOrContractMonth, contract, isBond) {
...
function readLastTradeDate(fields, contract, isBond) {
...
// ---------------------------------------------------------------------------
function decodeContractFromProto(proto) {
    const c = new Contract();
    if (proto.conId !== undefined)
        c.conId = proto.conId;
    if (proto.symbol !== undefined)
        c.symbol = proto.symbol;
    if (proto.secType !== undefined)
        c.secType = coerceSecType(proto.secType);
    if (proto.lastTradeDateOrContractMonth !== undefined)
        c.lastTradeDateOrContractMonth = proto.lastTradeDateOrContractMonth;
    if (proto.strike !== undefined)
        c.strike = proto.strike;
    if (proto.right !== undefined)
        c.right = proto.right;
    if (proto.multiplier !== undefined)
        c.multiplier = String(proto.multiplier);
    if (proto.exchange !== undefined)
        c.exchange = proto.exchange;
    if (proto.currency !== undefined)
        c.currency = proto.currency;
    if (proto.localSymbol !== undefined)
        c.localSymbol = proto.localSymbol;
    if (proto.tradingClass !== undefined)
        c.tradingClass = proto.tradingClass;
    if (proto.comboLegsDescrip !== undefined)
        c.comboLegsDescrip = proto.comboLegsDescrip;
    // combo legs
    if (proto.comboLegs && proto.comboLegs.length > 0) {
        c.comboLegs = proto.comboLegs.map((lp) => {
            const leg = new ComboLeg();
            if (lp.conId !== undefined)
                leg.conId = lp.conId;
            if (lp.ratio !== undefined)
                leg.ratio = lp.ratio;
            if (lp.action !== undefined)
                leg.action = lp.action;
            if (lp.exchange !== undefined)
                leg.exchange = lp.exchange;
            if (lp.openClose !== undefined)
                leg.openClose = lp.openClose;
            if (lp.shortSalesSlot !== undefined)
                leg.shortSaleSlot = lp.shortSalesSlot;
            if (lp.designatedLocation !== undefined)
                leg.designatedLocation = lp.designatedLocation;
            if (lp.exemptCode !== undefined)
    ...
    if (proto.deltaNeutralContract !== undefined) {
        ...
        if (proto.deltaNeutralContract.conId !== undefined)
            dnc.conId = proto.deltaNeutralContract.conId;
        if (proto.deltaNeutralContract.delta !== undefined)
            dnc.delta = proto.deltaNeutralContract.delta;
        if (proto.deltaNeutralContract.price !== undefined)
            dnc.price = proto.deltaNeutralContract.price;
        c.deltaNeutralContract = dnc;
    }
    if (proto.lastTradeDate !== undefined)
        c.lastTradeDate = proto.lastTradeDate;
    if (proto.primaryExch !== undefined)
        c.primaryExchange = proto.primaryExch;
    if (proto.issuerId !== undefined)
        c.issuerId = proto.issuerId;
    if (proto.description !== undefined)
        c.description = proto.description;
    return c;
}
function decodeTagValueList(protoMap) {
    const list = [];
    if (protoMap) {
...
function decodeContractDetailsFromProto(contractProto, detailsProto, isBond) {
    const cd = new ContractDetails();
    cd.contract = decodeContractFromProto(contractProto);
    if (detailsProto.marketName !== undefined)
        cd.marketName = detailsProto.marketName;
    if (detailsProto.minTick !== undefined)
        cd.minTick = parseFloat(detailsProto.minTick);
    if (detailsProto.priceMagnifier !== undefined)
        cd.priceMagnifier = detailsProto.priceMagnifier;
    if (detailsProto.orderTypes !== undefined)
        cd.orderTypes = detailsProto.orderTypes;
    if (detailsProto.validExchanges !== undefined)
        cd.validExchanges = detailsProto.validExchanges;
    if (detailsProto.underConId !== undefined)
        cd.underConId = detailsProto.underConId;
    if (detailsProto.longName !== undefined)
        cd.longName = detailsProto.longName;
    if (detailsProto.contractMonth !== undefined)
        cd.contractMonth = detailsProto.contractMonth;
    if (detailsProto.industry !== undefined)
        cd.industry = detailsProto.industry;
    if (detailsProto.category !== undefined)
        cd.category = detailsProto.category;
    if (detailsProto.subcategory !== undefined)
        cd.subcategory = detailsProto.subcategory;
    if (detailsProto.timeZoneId !== undefined)
        cd.timeZoneId = detailsProto.timeZoneId;
    if (detailsProto.tradingHours !== undefined)
        cd.tradingHours = detailsProto.tradingHours;
    if (detailsProto.liquidHours !== undefined)
        cd.liquidHours = detailsProto.liquidHours;
    if (detailsProto.evRule !== undefined)
        cd.evRule = detailsProto.evRule;
    if (detailsProto.evMultiplier !== undefined)
        cd.evMultiplier = detailsProto.evMultiplier;
    const secIdList = decodeTagValueList(detailsProto.secIdList);
    if (secIdList)
        cd.secIdList = secIdList;
    if (detailsProto.aggGroup !== undefined)
        cd.aggGroup = detailsProto.aggGroup;
    if (detailsProto.underSymbol !== undefined)
        cd.underSymbol = detailsProto.underSymbol;
    if (detailsProto.underSecType !== undefined)
        cd.underSecType = detailsProto.underSecType;
    if (detailsProto.marketRuleIds !== undefined)
        cd.marketRuleIds = detailsProto.marketRuleIds;
    if (detailsProto.realExpirationDate !== undefined)
        cd.realExpirationDate = detailsProto.realExpirationDate;
    if (detailsProto.stockType !== undefined)
        cd.stockType = detailsProto.stockType;
    if (detailsProto.minSize !== undefined)
        cd.minSize = new Decimal(detailsProto.minSize);
    if (detailsProto.sizeIncrement !== undefined)
        cd.sizeIncrement = new Decimal(detailsProto.sizeIncrement);
    if (detailsProto.suggestedSizeIncrement !== undefined)
        cd.suggestedSizeIncrement = new Decimal(detailsProto.suggestedSizeIncrement);
    if (detailsProto.minAlgoSize !== undefined)
        cd.minAlgoSize = new Decimal(detailsProto.minAlgoSize);
    if (detailsProto.lastPricePrecision !== undefined)
        cd.lastPricePrecision = new Decimal(detailsProto.lastPricePrecision);
    if (detailsProto.lastSizePrecision !== undefined)
        cd.lastSizePrecision = new Decimal(detailsProto.lastSizePrecision);
    setLastTradeDate(cd.contract.lastTradeDateOrContractMonth, cd, isBond);
    // Bond fields
    if (detailsProto.cusip !== undefined)
        cd.cusip = detailsProto.cusip;
    if (detailsProto.ratings !== undefined)
        cd.ratings = detailsProto.ratings;
    if (detailsProto.descAppend !== undefined)
        cd.descAppend = detailsProto.descAppend;
    if (detailsProto.bondType !== undefined)
        cd.bondType = detailsProto.bondType;
    if (detailsProto.coupon !== undefined)
        cd.coupon = detailsProto.coupon;
    if (detailsProto.couponType !== undefined)
        cd.couponType = detailsProto.couponType;
    if (detailsProto.callable !== undefined)
        cd.callable = detailsProto.callable;
    if (detailsProto.puttable !== undefined)
        cd.putable = detailsProto.puttable;
    if (detailsProto.convertible !== undefined)
        cd.convertible = detailsProto.convertible;
    if (detailsProto.issueDate !== undefined)
        cd.issueDate = detailsProto.issueDate;
    if (detailsProto.nextOptionDate !== undefined)
        cd.nextOptionDate = detailsProto.nextOptionDate;
    if (detailsProto.nextOptionType !== undefined)
        cd.nextOptionType = detailsProto.nextOptionType;
    if (detailsProto.nextOptionPartial !== undefined)
        cd.nextOptionPartial = detailsProto.nextOptionPartial;
    if (detailsProto.bondNotes !== undefined)
        cd.notes = detailsProto.bondNotes;
    // Fund fields
    if (detailsProto.fundName !== undefined)
        cd.fundName = detailsProto.fundName;
    if (detailsProto.fundFamily !== undefined)
        cd.fundFamily = detailsProto.fundFamily;
    if (detailsProto.fundType !== undefined)
        cd.fundType = detailsProto.fundType;
    if (detailsProto.fundFrontLoad !== undefined)
        cd.fundFrontLoad = detailsProto.fundFrontLoad;
    if (detailsProto.fundBackLoad !== undefined)
        cd.fundBackLoad = detailsProto.fundBackLoad;
    if (detailsProto.fundBackLoadTimeInterval !== undefined)
        cd.fundBackLoadTimeInterval = detailsProto.fundBackLoadTimeInterval;
    if (detailsProto.fundManagementFee !== undefined)
        cd.fundManagementFee = detailsProto.fundManagementFee;
    if (detailsProto.fundClosed !== undefined)
        cd.fundClosed = detailsProto.fundClosed;
    if (detailsProto.fundClosedForNewInvestors !== undefined)
        cd.fundClosedForNewInvestors = detailsProto.fundClosedForNewInvestors;
    if (detailsProto.fundClosedForNewMoney !== undefined)
        cd.fundClosedForNewMoney = detailsProto.fundClosedForNewMoney;
    if (detailsProto.fundNotifyAmount !== undefined)
        cd.fundNotifyAmount = detailsProto.fundNotifyAmount;
    if (detailsProto.fundMinimumInitialPurchase !== undefined)
        cd.fundMinimumInitialPurchase = detailsProto.fundMinimumInitialPurchase;
    if (detailsProto.fundMinimumSubsequentPurchase !== undefined)
        cd.fundSubsequentMinimumPurchase = detailsProto.fundMinimumSubsequentPurchase;
    if (detailsProto.fundBlueSkyStates !== undefined)
        cd.fundBlueSkyStates = detailsProto.fundBlueSkyStates;
    if (detailsProto.fundBlueSkyTerritories !== undefined)
        cd.fundBlueSkyTerritories = detailsProto.fundBlueSkyTerritories;
    if (detailsProto.fundDistributionPolicyIndicator !== undefined) {
        cd.fundDistributionPolicyIndicator = getEnumTypeFromString(FundDistributionPolicyIndicator, detailsProto.fundDistributionPolicyIndicator);
    }
    if (detailsProto.fundAssetType !== undefined) {
        cd.fundAssetType = getEnumTypeFromString(FundAssetType, detailsProto.fundAssetType);
    }
    ...
    if (detailsProto.eventContract1 !== undefined)
        cd.eventContract1 = detailsProto.eventContract1;
    if (detailsProto.eventContractDescription1 !== undefined)
        cd.eventContractDescription1 = detailsProto.eventContractDescription1;
    if (detailsProto.eventContractDescription2 !== undefined)
        cd.eventContractDescription2 = detailsProto.eventContractDescription2;
    return cd;
}
// ---------------------------------------------------------------------------
// Text handlers
// ---------------------------------------------------------------------------
function processContractDataMsg(d, fields) {
    ...
    if (d.serverVersion >= MIN_SERVER_VER_MD_SIZE_MULTIPLIER &&
    ...
    if (d.serverVersion >= MIN_SERVER_VER_FRACTIONAL_SIZE_SUPPORT &&
        d.serverVersion < MIN_SERVER_VER_SIZE_RULES) {
    ...
    if (d.serverVersion >= MIN_SERVER_VER_FUND_DATA_FIELDS &&
        ...
        contract.fundDistributionPolicyIndicator = getEnumTypeFromString(FundDistributionPolicyIndicator, decodeStr(fields));
        contract.fundAssetType = getEnumTypeFromString(FundAssetType, decodeStr(fields));
    }
...
function processBondContractDataMsg(d, fields) {
    decodeInt(fields); // msgId
    let version = 6;
    if (d.serverVersion < MIN_SERVER_VER_SIZE_RULES) {
        version = decodeInt(fields);
...
function processDeltaNeutralValidationMsg(d, fields) {
    decodeInt(fields); // msgId
    decodeInt(fields); // version
    const reqId = decodeInt(fields);
...
function processMarketRuleMsg(d, fields) {
    decodeInt(fields); // msgId
...
// Protobuf handlers
// ---------------------------------------------------------------------------
function processContractDataMsgProtoBuf(d, buf) {
...
function processBondContractDataMsgProtoBuf(d, buf) {
    const proto = ContractDataProto.decode(buf);
    const reqId = proto.reqId ?? NO_VALID_ID;
    if (proto.contract === undefined || proto.contractDetails === undefined) {
        return;
...
function processContractDataEndMsgProtoBuf(d, buf) {
    const proto = ContractDataEndProto.decode(buf);
    const reqId = proto.reqId ?? NO_VALID_ID;
    d.wrapper.contractDetailsEnd(reqId);
}
function processSymbolSamplesMsgProtoBuf(d, buf) {
...
function processMarketRuleMsgProtoBuf(d, buf) {
    ...
    if (proto.priceIncrements) {
        for (const pip of proto.priceIncrements) {
            ...
            if (pip.lowEdge !== undefined)
                pi.lowEdge = pip.lowEdge;
            if (pip.increment !== undefined)
                pi.increment = pip.increment;
...
// Registration
// ---------------------------------------------------------------------------
export function applyContractHandlers(decoder) {
    // Text handlers
