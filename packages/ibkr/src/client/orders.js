...
export function applyOrders(Client) {
    Client.prototype.placeOrder = function (orderId, contract, order) {
        if (!this.requireConnected(orderId))
            return;
        ...
        try {
            ...
            flds.push(makeField(order.tif), makeField(order.ocaGroup), makeField(order.account), makeField(order.openClose), makeField(order.origin), makeField(order.orderRef), makeField(order.transmit), makeField(order.parentId), makeField(order.blockOrder), makeField(order.sweepToFill), makeField(order.displaySize), makeField(order.triggerMethod), makeField(order.outsideRth), makeField(order.hidden));
            ...
            if (contract.secType === 'BAG') {
                ...
                if (comboLegsCount > 0 && contract.comboLegs) {
                    for (const comboLeg of contract.comboLegs) {
                        flds.push(makeField(comboLeg.conId), makeField(comboLeg.ratio), makeField(comboLeg.action), makeField(comboLeg.exchange), makeField(comboLeg.openClose), makeField(comboLeg.shortSaleSlot), makeField(comboLeg.designatedLocation));
            ...
            flds.push(makeField(''), makeField(order.discretionaryAmt), makeField(order.goodAfterTime), makeField(order.goodTillDate), makeField(order.faGroup), makeField(order.faMethod), makeField(order.faPercentage));
            ...
            flds.push(makeField(order.shortSaleSlot), makeField(order.designatedLocation));
            if (this.serverVersion() >= SV.MIN_SERVER_VER_SSHORTX_OLD) {
                flds.push(makeField(order.exemptCode));
            }
            flds.push(makeField(order.ocaType));
            flds.push(makeField(order.rule80A), makeField(order.settlingFirm), makeField(order.allOrNone), makeFieldHandleEmpty(order.minQty), makeFieldHandleEmpty(order.percentOffset), makeField(false), makeField(false), makeFieldHandleEmpty(UNSET_DOUBLE), makeField(order.auctionStrategy), makeFieldHandleEmpty(order.startingPrice), makeFieldHandleEmpty(order.stockRefPrice), makeFieldHandleEmpty(order.delta), makeFieldHandleEmpty(order.stockRangeLower), makeFieldHandleEmpty(order.stockRangeUpper), makeField(order.overridePercentageConstraints), 
            // Volatility orders
            makeFieldHandleEmpty(order.volatility), makeFieldHandleEmpty(order.volatilityType), makeField(order.deltaNeutralOrderType), makeFieldHandleEmpty(order.deltaNeutralAuxPrice));
            if (this.serverVersion() >= SV.MIN_SERVER_VER_DELTA_NEUTRAL_CONID && order.deltaNeutralOrderType) {
                flds.push(makeField(order.deltaNeutralConId), makeField(order.deltaNeutralSettlingFirm), makeField(order.deltaNeutralClearingAccount), makeField(order.deltaNeutralClearingIntent));
            }
            if (this.serverVersion() >= SV.MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE && order.deltaNeutralOrderType) {
                flds.push(makeField(order.deltaNeutralOpenClose), makeField(order.deltaNeutralShortSale), makeField(order.deltaNeutralShortSaleSlot), makeField(order.deltaNeutralDesignatedLocation));
            }
            flds.push(makeField(order.continuousUpdate), makeFieldHandleEmpty(order.referencePriceType), makeFieldHandleEmpty(order.trailStopPrice));
            if (this.serverVersion() >= SV.MIN_SERVER_VER_TRAILING_PERCENT) {
                flds.push(makeFieldHandleEmpty(order.trailingPercent));
            }
            // SCALE orders
            if (this.serverVersion() >= SV.MIN_SERVER_VER_SCALE_ORDERS2) {
                flds.push(makeFieldHandleEmpty(order.scaleInitLevelSize), makeFieldHandleEmpty(order.scaleSubsLevelSize));
            }
            else {
                flds.push(makeField(''), // for not supported scaleNumComponents
            ...
            if (this.serverVersion() >= SV.MIN_SERVER_VER_SCALE_ORDERS3 &&
                ...
                flds.push(makeFieldHandleEmpty(order.scalePriceAdjustValue), makeFieldHandleEmpty(order.scalePriceAdjustInterval), makeFieldHandleEmpty(order.scaleProfitOffset), makeField(order.scaleAutoReset), makeFieldHandleEmpty(order.scaleInitPosition), makeFieldHandleEmpty(order.scaleInitFillQty), makeField(order.scaleRandomPercent));
            }
            if (this.serverVersion() >= SV.MIN_SERVER_VER_SCALE_TABLE) {
                flds.push(makeField(order.scaleTable), makeField(order.activeStartTime), makeField(order.activeStopTime));
            }
            // HEDGE orders
            if (this.serverVersion() >= SV.MIN_SERVER_VER_HEDGE_ORDERS) {
            ...
            if (this.serverVersion() >= SV.MIN_SERVER_VER_PTA_ORDERS) {
                flds.push(makeField(order.clearingAccount), makeField(order.clearingIntent));
            }
            if (this.serverVersion() >= SV.MIN_SERVER_VER_NOT_HELD) {
            ...
            if (this.serverVersion() >= SV.MIN_SERVER_VER_RANDOMIZE_SIZE_AND_PRICE) {
                flds.push(makeField(order.randomizeSize), makeField(order.randomizePrice));
            }
            if (this.serverVersion() >= SV.MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
                if (isPegBenchOrder(order.orderType)) {
                    flds.push(makeField(order.referenceContractId), makeField(order.isPeggedChangeAmountDecrease), makeField(order.peggedChangeAmount), makeField(order.referenceChangeAmount), makeField(order.referenceExchangeId));
                }
                ...
                if (order.conditions.length > 0) {
                    ...
                    flds.push(makeField(order.conditionsIgnoreRth), makeField(order.conditionsCancelOrder));
                }
                flds.push(makeField(order.adjustedOrderType), makeField(order.triggerPrice), makeField(order.lmtPriceOffset), makeField(order.adjustedStopPrice), makeField(order.adjustedStopLimitPrice), makeField(order.adjustedTrailingAmount), makeField(order.adjustableTrailingUnit));
            }
            if (this.serverVersion() >= SV.MIN_SERVER_VER_EXT_OPERATOR) {
                flds.push(makeField(order.extOperator));
            }
            if (this.serverVersion() >= SV.MIN_SERVER_VER_SOFT_DOLLAR_TIER) {
                flds.push(makeField(order.softDollarTier.name), makeField(order.softDollarTier.val));
            }
            if (this.serverVersion() >= SV.MIN_SERVER_VER_CASH_QTY) {
            ...
            if (this.serverVersion() >= SV.MIN_SERVER_VER_PRICE_MGMT_ALGO) {
                flds.push(makeFieldHandleEmpty(order.usePriceMgmtAlgo == null
                    ? UNSET_INTEGER
    ...
    Client.prototype.cancelOrder = function (orderId, orderCancel) {
        if (!this.requireConnected(orderId))
            return;
    ...
    Client.prototype.reqOpenOrders = function () {
    ...
    Client.prototype.reqAutoOpenOrders = function (bAutoBind) {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.reqAllOpenOrders = function () {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.reqGlobalCancel = function (orderCancel) {
        if (!this.requireConnected())
            return;
        if (this.serverVersion() < SV.MIN_SERVER_VER_CME_TAGGING_FIELDS && (orderCancel.extOperator !== '' || orderCancel.manualOrderIndicator !== UNSET_INTEGER)) {
    ...
    Client.prototype.reqIds = function (numIds) {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.reqCompletedOrders = function (apiOnly) {
        if (!this.requireConnected())
            return;
        try {
