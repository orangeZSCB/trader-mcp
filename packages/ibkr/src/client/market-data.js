...
export function applyMarketData(Client) {
    Client.prototype.reqCurrentTime = function () {
    ...
    Client.prototype.reqCurrentTimeInMillis = function () {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.setServerLogLevel = function (logLevel) {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.reqMktData = function (reqId, contract, genericTickList, snapshot, regulatorySnapshot, mktDataOptions) {
    ...
    Client.prototype.cancelMktData = function (reqId) {
        if (!this.requireConnected(reqId))
            return;
        try {
    ...
    Client.prototype.reqMarketDataType = function (marketDataType) {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.reqSmartComponents = function (reqId, bboExchange) {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.reqMarketRule = function (marketRuleId) {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.reqTickByTickData = function (reqId, contract, tickType, numberOfTicks, ignoreSize) {
        if (!this.requireConnected())
            return;
        try {
            const flds = [
    ...
    Client.prototype.cancelTickByTickData = function (reqId) {
        if (!this.requireConnected())
            return;
        try {
    ...
    Client.prototype.calculateImpliedVolatility = function (reqId, contract, optionPrice, underPrice, implVolOptions) {
        if (!this.requireConnected(reqId))
            return;
        try {
            const flds = [
    ...
    Client.prototype.cancelCalculateImpliedVolatility = function (reqId) {
        if (!this.requireConnected(reqId))
            return;
        try {
    ...
    Client.prototype.calculateOptionPrice = function (reqId, contract, volatility, underPrice, optPrcOptions) {
        if (!this.requireConnected(reqId))
            return;
        try {
            const flds = [
    ...
    Client.prototype.cancelCalculateOptionPrice = function (reqId) {
        if (!this.requireConnected(reqId))
            return;
        try {
    ...
    Client.prototype.exerciseOptions = function (reqId, contract, exerciseAction, exerciseQuantity, account, override, manualOrderTime, customerAccount, professionalCustomer) {
        if (!this.requireConnected(reqId))
            return;
        try {
            ...
            if (this.serverVersion() >= SV.MIN_SERVER_VER_TRADING_CLASS)
                flds.push(makeField(contract.conId));
            flds.push(makeField(contract.symbol), makeField(contract.secType), makeField(contract.lastTradeDateOrContractMonth), makeFieldHandleEmpty(contract.strike), makeField(contract.right), makeField(contract.multiplier), makeField(contract.exchange), makeField(contract.currency), makeField(contract.localSymbol));
            if (this.serverVersion() >= SV.MIN_SERVER_VER_TRADING_CLASS)
                flds.push(makeField(contract.tradingClass));
            flds.push(makeField(exerciseAction), makeField(exerciseQuantity), makeField(account), makeField(override));
            if (this.serverVersion() >= SV.MIN_SERVER_VER_MANUAL_ORDER_TIME_EXERCISE_OPTIONS)
                flds.push(makeField(manualOrderTime));
            if (this.serverVersion() >= SV.MIN_SERVER_VER_CUSTOMER_ACCOUNT)
                flds.push(makeField(customerAccount));
            if (this.serverVersion() >= SV.MIN_SERVER_VER_PROFESSIONAL_CUSTOMER)
                flds.push(makeField(professionalCustomer));
