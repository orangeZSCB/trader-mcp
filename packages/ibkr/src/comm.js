 ...
 */
export function makeMsgProto(msgId, protobufData) {
 ...
 */
export function makeMsg(msgId, useRawIntMsgId, text) {
    let payload;
    if (useRawIntMsgId) {
        const msgIdBuf = Buffer.alloc(4);
        msgIdBuf.writeUInt32BE(msgId);
 ...
 */
export function makeInitialMsg(text) {
...
export function makeField(val) {
    ...
    if (typeof val === 'string' && val.length > 0 && !isAsciiPrintable(val)) {
        throw new ClientException(INVALID_SYMBOL.code(), INVALID_SYMBOL.msg(), val);
    }
    // bool → int
 ...
 */
export function makeFieldHandleEmpty(val) {
    if (val === null || val === undefined) {
        throw new Error('Cannot send None to TWS');
    }
    if (val instanceof Decimal) {
