...
describe.runIf(available)('TWS Order precision — lmtPrice Decimal round-trip', () => {
    ...
    it('IEEE 754 trap value (0.1 + 0.2) stays clean when sent as Decimal', async () => {
        ...
        await waitFor(() => results.openOrders.has(orderId) || results.errors.length > errorCountBefore, 5000);
        if (results.openOrders.has(orderId)) {
            ...
            console.log('  [note] TWS rejected lmtPrice=0.3 (expected for AAPL):', recent.map(e => `${e.code}: ${e.msg}`).join(' | '));
        }
    });
    it('sub-tick precision: TWS rejects or rounds, but our wire stays exact', async () => {
        ...
        if (results.openOrders.has(orderId)) {
            ...
            const recent = results.errors.slice(errorCountBefore);
            console.log('  [note] sub-tick rejected (expected behaviour):', recent.map(e => `${e.code}: ${e.msg}`).join(' | '));
...
//# sourceMappingURL=order-precision.e2e.spec.js.map