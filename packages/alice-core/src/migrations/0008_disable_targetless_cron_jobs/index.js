 ...
 */
export async function disableTargetlessCronJobs(jobsFilePath = DEFAULT_JOBS_PATH) {
    let raw;
    try {
...
export const migration = {
    ...
    summary: 'Disable enabled cron jobs that have no target workspace (legacy AgentWork-era jobs) so they stop firing into the retired path.',
    rationale: 'Cron now dispatches headless Workspace runs (workspaceId + agent). Targetless jobs would error loudly on every fire; disable them so the user can re-target or delete.',
    up: async () => {
