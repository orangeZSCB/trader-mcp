...
const BAR_INTERVALS = ['1m', '5m', '15m', '30m', '1h', '4h', '1d', '1w'];
function toBarInterval(interval) {
    return BAR_INTERVALS.includes(interval)
        ? interval
        : '1d';
}
...
function secTypeToAssetClass(secType) {
    switch ((secType ?? '').toUpperCase()) {
        case 'STK':
        case 'ETF':
        case 'WAR': return 'equity';
        case 'CRYPTO':
        case 'CRYPTO_PERP': return 'crypto';
...
function getCalendarDays(interval) {
...
function buildMeta(symbol, bars, extra) {
    return {
        symbol,
...
export function createBarService(deps) {
    // -------- vendor fetch --------
    async function getVendorBars(provider, assetClass, symbol, opts) {
        ...
        const p = (extra) => ({ symbol, start_date, provider, ...(end_date ? { end_date } : {}), ...extra });
        let raw;
        switch (assetClass) {
            case 'equity':
        ...
        let bars = raw.filter(isFullBar);
        if (end_date)
            bars = bars.filter((b) => b.date.slice(0, 10) <= end_date);
    ...
    // -------- uta (broker) fetch --------
    async function getUtaBars(sourceId, barId, opts) {
        const acct = await deps.utaManager.get(sourceId);
        if (!acct)
            throw new Error(`UTA source "${sourceId}" not found for barId "${barId}"`);
        const params = {
    ...
    return {
        async searchBarSources(query, opts) {
            ...
            if (utaRes.status === 'fulfilled') {
                for (const hit of utaRes.value) {
                    ...
                    if (!barId)
                        continue; // need the operational identity to fetch later
        ...
        async getBars(ref, opts) {
            ...
            if (!parsed)
                throw new Error(`Invalid barId "${ref.barId}" (expected "sourceId|nativeSymbol")`);
            const isUta = await deps.utaManager.has(parsed.sourceId);
            if (isUta)
                return getUtaBars(parsed.sourceId, ref.barId, opts);
            // vendor barId — needs an assetClass to route to the right client
            if (!ref.assetClass) {
                throw new Error(`Vendor barId "${ref.barId}" needs an assetClass to route. Pass { barId, assetClass } or use { symbol, assetClass }.`);
            }
