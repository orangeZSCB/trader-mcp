...
import type { EquityClientLike, CryptoClientLike, CurrencyClientLike, CommodityClientLike } from '../client/types.js';
export interface BarRef {
    sourceId: string;
    nativeSymbol: string;
}
/** Split a barId on the FIRST `|` (nativeKey may itself contain separators). */
export declare function parseBarId(barId: string): BarRef | null;
export declare function formatBarId(sourceId: string, nativeSymbol: string): string;
...
export interface BarService {
    searchBarSources(query: string, opts?: {
        limit?: number;
    }): Promise<BarSourceCandidate[]>;
