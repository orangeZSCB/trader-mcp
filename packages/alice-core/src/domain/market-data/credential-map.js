...
function applyMapping(providerKeys, table) {
    if (!providerKeys)
        return {};
    const mapped = {};
 ...
 */
export function buildSDKCredentials(providerKeys, hub) {
    ...
    if (hub?.enabled && hub.baseUrl) {
        for (const field of ['federal_reserve_api_key', 'eia_api_key', 'bls_api_key']) {
            if (!mapped[field])
                mapped[field] = `hub:${hub.baseUrl}`;
        }
    }
    return mapped;
}
//# sourceMappingURL=credential-map.js.map