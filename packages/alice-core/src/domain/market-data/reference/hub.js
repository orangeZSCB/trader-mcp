...
export function createHubFetcher(cfg, opts) {
    ...
    return async function hubBoard(board) {
        if (!cfg?.enabled || !cfg.baseUrl)
            return null;
        if (Date.now() < (downUntil.get(board) ?? 0))
            return null;
        let res;
        try {
        ...
        try {
            if (!res.ok)
                return null; // board-specific upstream failure; no breaker
            const data = await res.json();
            if (!data || typeof data !== 'object' || !('meta' in data))
                return null;
            const payload = data;
            return { ...payload, meta: { ...payload.meta, origin: 'hub' } };
        }
        catch {
            return null;
        }
...
export function markLocal(board) {
    return { ...board, meta: { ...board.meta, origin: 'local' } };
}
