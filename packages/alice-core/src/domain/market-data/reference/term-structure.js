...
export async function fetchTermStructure(derivativesClient) {
    const settled = await Promise.allSettled(SYMBOLS.map((symbol) => derivativesClient.getFuturesCurve({ provider: 'deribit', symbol })));
    ...
    settled.forEach((r, i) => {
        ...
        const points = rows
            ...
            const basis = spot != null && row.price != null && days != null && days >= MIN_DAYS_FOR_BASIS
