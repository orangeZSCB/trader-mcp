...
export async function fetchFedBoard(economyClient) {
    ...
    if (holdings.status === 'fulfilled') {
        ...
        const points = (field) => holdings.value
            .map((r) => ({ date: String(r.date), value: r[field] }))
            .filter((p) => typeof p.value === 'number')
            .map((p) => ({ date: p.date, value: p.value * 1e6 }));
        cards.push(card('WALCL', 'Total Assets', points('total_assets')), card('TREAST', 'Treasuries Held', points('treasury_holding_value')), card('WSHOMCB', 'MBS Held', points('mbs_holding_value')));
    }
    else {
    ...
    if (dealers.status === 'fulfilled') {
        const points = (field) => dealers.value
            .map((r) => ({ date: String(r.date), value: r[field] }))
            .filter((p) => typeof p.value === 'number')
            .map((p) => ({ date: p.date, value: p.value * 1e6 })); // millions → absolute
        cards.push(card('PD_NET', 'Dealer Net Positions (Total)', points('total_net_position')), card('PD_UST', 'Dealer Net Treasuries', points('treasury_total')));
    }
    else {
