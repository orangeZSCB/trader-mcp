...
function truncateMetadata(metadata, maxLength = 40) {
...
export async function globRss(context, options) {
    const news = await context.getNews();
    const regex = new RegExp(options.pattern, 'i');
    const results = [];
    for (const item of news) {
        if (options.metadataFilter && !matchesMetadataFilter(item.metadata, options.metadataFilter))
            continue;
...
export async function grepRss(context, options) {
    const news = await context.getNews();
...
export async function readRss(context, options) {
    const news = await context.getNews();
    return news.find((item) => item.id === options.id) ?? null;
}
// ==================== AI Tool factory ====================
export function createNewsArchiveTools(provider) {
    return {
        globRss: tool({
...
Example: globRss({ pattern: "BTC|Bitcoin", lookback: "1d" })`,
            ...
            execute: async ({ pattern, lookback, metadataFilter, limit }) => {
                return globRss({ getNews: () => provider.getNewsV2({ endTime: new Date(), lookback, limit: NEWS_LIMIT }) }, { pattern, metadataFilter, limit });
            },
        }),
        grepRss: tool({
...
Example: grepRss({ pattern: "interest rate", lookback: "2d" })`,
            ...
            execute: async ({ pattern, lookback, contextChars, metadataFilter, limit }) => {
                return grepRss({ getNews: () => provider.getNewsV2({ endTime: new Date(), lookback, limit: NEWS_LIMIT }) }, { pattern, contextChars, metadataFilter, limit });
            },
        }),
        readRss: tool({
...
to find the item (no need to repeat it).`,
            ...
            execute: async ({ id }) => {
                const result = await readRss({ getNews: () => provider.getNewsV2({ endTime: new Date(), limit: NEWS_LIMIT }) }, { id });
