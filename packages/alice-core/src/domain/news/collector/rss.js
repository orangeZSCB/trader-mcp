...
import { computeDedupKey } from '../store.js';
export class NewsCollector {
    ...
    start() {
        this.fetchAll().catch((err) => console.warn(`news-collector: initial fetch failed: ${err instanceof Error ? err.message : err}`));
        this.timer = setInterval(() => this.fetchAll().catch((err) => console.warn(`news-collector: periodic fetch failed: ${err instanceof Error ? err.message : err}`)), this.intervalMs);
    }
    ...
    async _fetchAllImpl() {
        ...
        for (const feed of activeFeeds) {
            try {
                ...
                console.warn(`news-collector: failed to fetch ${feed.name} (${feed.url}): ${err instanceof Error ? err.message : err}`);
            }
        }
        if (totalNew > 0) {
            console.log(`news-collector: fetched ${totalItems} items from ${activeFeeds.length} active feeds, ${totalNew} new`);
        }
