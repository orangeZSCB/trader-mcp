 ...
 */
export async function CLOSE(symbol, interval, context) {
    const { data, meta } = await context.getHistoricalData(symbol, interval);
    return { values: data.map((d) => d.close), source: meta };
}
export async function HIGH(symbol, interval, context) {
    const { data, meta } = await context.getHistoricalData(symbol, interval);
    return { values: data.map((d) => d.high), source: meta };
}
export async function LOW(symbol, interval, context) {
    const { data, meta } = await context.getHistoricalData(symbol, interval);
    return { values: data.map((d) => d.low), source: meta };
}
export async function OPEN(symbol, interval, context) {
    const { data, meta } = await context.getHistoricalData(symbol, interval);
    return { values: data.map((d) => d.open), source: meta };
}
export async function VOLUME(symbol, interval, context) {
    const { data, meta } = await context.getHistoricalData(symbol, interval);
