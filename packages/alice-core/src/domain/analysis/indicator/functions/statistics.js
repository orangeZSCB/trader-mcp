...
export function ROC(data, period) {
    const v = toValues(data);
...
export function ZSCORE(data, period) {
...
export function SLOPE(data, period) {
...
export function HIGHEST(data, period) {
    const v = toValues(data).slice(-period);
    if (v.length === 0)
...
export function LOWEST(data, period) {
    const v = toValues(data).slice(-period);
    if (v.length === 0)
