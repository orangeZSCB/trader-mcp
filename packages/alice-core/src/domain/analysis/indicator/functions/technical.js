...
export function BBANDS(data, period = 20, stdDevMultiplier = 2) {
    const v = toValues(data);
    if (v.length < period) {
...
export function MACD(data, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
    ...
    if (v.length < slowPeriod + signalPeriod) {
        throw new Error(`MACD requires at least ${slowPeriod + signalPeriod} data points, got ${v.length}`);
    }
...
export function OBV(closes, volumes) {
    ...
    if (c.length !== vol.length || c.length < 2) {
        throw new Error(`OBV requires closes and volumes of equal length (>= 2), got ${c.length} and ${vol.length}`);
    }
    let obv = 0;
