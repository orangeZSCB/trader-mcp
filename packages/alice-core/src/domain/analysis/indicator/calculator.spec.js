...
const mockContext = {
    getHistoricalData: async (_symbol, _interval) => ({
        data: mockData,
        meta: {
            symbol: _symbol,
...
async function calc(formula, precision) {
...
describe('statistics', () => {
    ...
    it('EMA', async () => {
        const result = (await calc("EMA(CLOSE('AAPL', '1d'), 10)")).value;
...
describe('technical indicators', () => {
    it('RSI returns 0-100, trending up → high RSI', async () => {
        const result = (await calc("RSI(CLOSE('AAPL', '1d'), 14)")).value;
        expect(result).toBeGreaterThanOrEqual(0);
        expect(result).toBeLessThanOrEqual(100);
        expect(result).toBeGreaterThan(90);
    });
    it('BBANDS returns { upper, middle, lower }', async () => {
        const result = (await calc("BBANDS(CLOSE('AAPL', '1d'), 20, 2)")).value;
    ...
    it('MACD returns { macd, signal, histogram }', async () => {
        const result = (await calc("MACD(CLOSE('AAPL', '1d'), 12, 26, 9)")).value;
    ...
    it('ATR returns positive number', async () => {
        const result = (await calc("ATR(HIGH('AAPL', '1d'), LOW('AAPL', '1d'), CLOSE('AAPL', '1d'), 14)")).value;
        expect(typeof result).toBe('number');
...
describe('volume indicators', () => {
    it('RVOL returns latest volume relative to its baseline (>1 here, volume trends up)', async () => {
        const result = (await calc("RVOL(VOLUME('AAPL', '1d'), 20)")).value;
        expect(typeof result).toBe('number');
    ...
    it('OBV sums volume on up-closes — mock rises monotonically, so = Σ volume[1..49]', async () => {
        ...
        const result = (await calc("OBV(CLOSE('AAPL', '1d'), VOLUME('AAPL', '1d'))")).value;
        expect(result).toBe(59770);
    });
    it('MFI is 100 when typical price rises monotonically (all positive money flow)', async () => {
        const result = (await calc("MFI(HIGH('AAPL', '1d'), LOW('AAPL', '1d'), CLOSE('AAPL', '1d'), VOLUME('AAPL', '1d'), 14)")).value;
        expect(result).toBe(100);
    });
    it('VWAP returns a volume-weighted price inside the price range', async () => {
        const result = (await calc("VWAP(HIGH('AAPL', '1d'), LOW('AAPL', '1d'), CLOSE('AAPL', '1d'), VOLUME('AAPL', '1d'))")).value;
        expect(typeof result).toBe('number');
...
describe('complex expressions', () => {
    it('price deviation from MA (%)', async () => {
        const result = (await calc("(CLOSE('AAPL', '1d')[-1] - SMA(CLOSE('AAPL', '1d'), 50)) / SMA(CLOSE('AAPL', '1d'), 50) * 100")).value;
...
describe('precision', () => {
    ...
    it('precision applies to record values', async () => {
        const result = (await calc("BBANDS(CLOSE('AAPL', '1d'), 20, 2)", 2)).value;
