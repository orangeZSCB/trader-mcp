...
import { computeSectorRotation, GICS_SECTOR_ETFS, BENCHMARK_SYMBOL, } from './sector-rotation';
const BARS = 150;
const d = (i) => new Date(Date.UTC(2025, 0, 1) + i * 86400000).toISOString().slice(0, 10);
function series(closeFn, volFn) {
    return Array.from({ length: BARS }, (_, i) => {
        const close = closeFn(i);
        return { date: d(i), open: close, high: close, low: close, close, volume: volFn(i) };
    });
}
const flat = () => series(() => 100, () => 1000);
// XLK — rotating in: flat then a sharp last-20-bar rise + volume surge.
const rotatingIn = () => series((i) => (i <= 129 ? 100 : 100 + (i - 129) * 2), (i) => (i <= 129 ? 1000 : 1000 + (i - 129) * 400));
// XLU — rotating out: slow rise then a last-20-bar decline + volume fade.
const rotatingOut = () => series((i) => (i <= 129 ? 100 + i * 0.2 : 100 + 129 * 0.2 - (i - 129) * 1.5), (i) => (i <= 129 ? 1000 : Math.max(100, 1000 - (i - 129) * 40)));
function buildHistories() {
    const h = { [BENCHMARK_SYMBOL]: flat() };
...
//# sourceMappingURL=sector-rotation.spec.js.map