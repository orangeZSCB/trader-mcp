...
const SERIES_COLUMNS = ['open', 'high', 'low', 'close', 'volume'];
...
const MAX_PANEL = 50;
export async function evaluate(program, deps) {
    const env = new Map();
    const dataRange = {};
    const fetchBars = async (barId, opts, assetClass) => {
    ...
    const evalExpr = async (e) => {
        switch (e.type) {
            ...
            case 'binary': {
                ...
                if (e.op === '/' && r === 0)
                    throw new CalcError({ kind: 'type', message: 'Division by zero', line: e.pos.line });
            ...
            case 'attr': {
                ...
                if (!SERIES_COLUMNS.includes(e.name)) {
                ...
                const values = obj.r.bars.map((b) => Number(b[e.name] ?? 0));
            ...
            case 'index': {
                ...
                if (!Number.isInteger(idx))
                    throw new CalcError({ kind: 'type', message: 'Index must be an integer', line: e.pos.line });
                ...
                if (at < 0 || at >= arr.length)
                    throw new CalcError({ kind: 'insufficient-bars', message: `Index ${idx} out of range — the series has ${arr.length} bars`, line: e.pos.line });
            ...
            case 'list': {
                if (e.elements.length > MAX_PANEL)
                    throw new CalcError({ kind: 'type', message: `A panel takes at most ${MAX_PANEL} entries, got ${e.elements.length}`, line: e.pos.line });
                const items = [];
                for (const el of e.elements)
                    items.push(asLeaf(await evalExpr(el), el.pos.line));
                return { k: 'list', items };
            }
            case 'dict': {
                if (e.entries.length > MAX_PANEL)
                    throw new CalcError({ kind: 'type', message: `A panel takes at most ${MAX_PANEL} entries, got ${e.entries.length}`, line: e.pos.line });
                const map = {};
                for (const ent of e.entries)
                    map[ent.key] = asLeaf(await evalExpr(ent.value), ent.value.pos.line);
                return { k: 'dict', map };
            }
        }
    };
    const evalBars = async (e) => {
        const pos = e.args.filter((a) => !a.name).map((a) => a.value);
        const kw = new Map(e.args.filter((a) => a.name).map((a) => [a.name, a.value]));
        if (pos.length < 2)
            throw new CalcError({ kind: 'arity', message: 'bars(barId, interval, ...) needs a barId and an interval', line: e.pos.line, col: e.pos.col, suggestion: 'e.g. bars("alpaca-paper|AAPL", "1d", count=250)' });
        const barId = asStr(await evalExpr(pos[0]), 'bars', 1, e.pos.line);
        const interval = asStr(await evalExpr(pos[1]), 'bars', 2, e.pos.line);
        const opts = { interval };
        const count = kw.get('count');
        if (count)
            opts.count = asNum(await evalExpr(count), 'count', e.pos.line);
        const asOf = kw.get('asOf');
        if (asOf)
            opts.asOf = asStr(await evalExpr(asOf), 'asOf', 1, e.pos.line);
        const start = kw.get('start');
        if (start)
            opts.start = asStr(await evalExpr(start), 'start', 1, e.pos.line);
        const end = kw.get('end');
        if (end)
            opts.end = asStr(await evalExpr(end), 'end', 1, e.pos.line);
        const assetKw = kw.get('asset');
        const assetClass = assetKw ? asStr(await evalExpr(assetKw), 'asset', 1, e.pos.line) : undefined;
        let r;
        try {
        ...
        if (r.bars.length === 0)
            throw new CalcError({ kind: 'insufficient-bars', message: `bars("${barId}", "${interval}") returned no data`, line: e.pos.line, col: e.pos.col });
        return { k: 'series', barId, r };
    };
    const evalFunction = async (e) => {
        ...
        if (e.args.some((a) => a.name))
            throw new CalcError({ kind: 'type', message: `${e.callee}(...) takes positional arguments only`, line: e.pos.line, col: e.pos.col });
...
function asStr(v, fn, argIdx, line) {
    if (v.k === 'str')
        return v.v;
...
function buildFunctions() {
    const col = (a, fn, i, line) => {
        if (!a || a.k !== 'col')
            throw new CalcError({ kind: 'type', message: `${fn} arg ${i} must be a series column (e.g. s.close)`, line });
        return a.col;
    };
    const num = (a, fn, i, line) => {
        if (!a || a.k !== 'num')
            throw new CalcError({ kind: 'type', message: `${fn} arg ${i} must be a number`, line });
        return a.v;
    };
    const arity = (args, min, max, fn, line) => {
        if (args.length < min || args.length > max)
            throw new CalcError({ kind: 'arity', message: `${fn} expects ${min === max ? min : `${min}-${max}`} argument${max === 1 ? '' : 's'}, got ${args.length}`, line });
    };
    const needBars = (c, period, fn, line) => {
        if (c.values.length < period)
            throw new CalcError({ kind: 'insufficient-bars', message: `${fn}(period=${period}) needs ≥${period} bars, but the series has ${c.values.length}. Raise count= on bars(...) or shorten the period.`, line });
    };
    const n = (x) => ({ k: 'num', v: x });
    const rec = (x) => ({ k: 'rec', v: x });
    return {
        ...
        bbands: (f, a, l) => { arity(a, 2, 3, f, l); const c = col(a[0], f, 1, l), p = num(a[1], f, 2, l), s = a[2] ? num(a[2], f, 3, l) : 2; needBars(c, p, f, l); return rec(Tech.BBANDS(c, p, s)); },
        macd: (f, a, l) => { arity(a, 4, 4, f, l); const c = col(a[0], f, 1, l), fa = num(a[1], f, 2, l), sl = num(a[2], f, 3, l), si = num(a[3], f, 4, l); needBars(c, sl + si, f, l); return rec(Tech.MACD(c, fa, sl, si)); },
        ...
        zscore: (f, a, l) => { arity(a, 1, 2, f, l); const c = col(a[0], f, 1, l), p = a[1] ? num(a[1], f, 2, l) : undefined; if (p)
            needBars(c, p, f, l); return n(Stat.ZSCORE(c, p)); },
