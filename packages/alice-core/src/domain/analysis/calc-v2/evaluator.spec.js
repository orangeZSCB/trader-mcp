...
function mockBars(closes, barId = 'yfinance|AAPL') {
...
const run = (script, svc) => runScript(script, { barService: svc });
describe('calc-v2 evaluator', () => {
    ...
    it('returns a labeled panel (dict) — the multi-timeframe case', async () => {
        const r = await run(`h1 = bars("x","1h",asset="crypto")\nh4 = bars("x","4h",asset="crypto")\n{ "1h": sma(h1.close, 3), "4h": sma(h4.close, 3) }`, mockBars([1, 2, 3, 4, 5]));
        expect(r.error).toBeUndefined();
    ...
    it('a panel entry can be an indicator record (nested)', async () => {
        ...
        expect(r.value.bb).toHaveProperty('upper');
    });
});
