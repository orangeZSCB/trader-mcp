...
class Parser {
    toks;
    p = 0;
    constructor(src) { this.toks = tokenize(src); }
    peek(ahead = 0) { return this.toks[Math.min(this.p + ahead, this.toks.length - 1)]; }
    next() { return this.toks[this.p++]; }
    at(t) { return this.peek().type === t; }
    eat(t) {
        if (!this.at(t)) {
            const got = this.peek();
    ...
    parsePrimary() {
        ...
        if (t.type === 'num') {
            this.next();
            return { type: 'num', value: Number(t.value), pos: t.pos };
        }
        if (t.type === 'str') {
            this.next();
        ...
        if (t.type === 'name') {
            ...
            this.next();
            return { type: 'name', id: t.value, pos: t.pos };
        }
    ...
    parseDictKey() {
        const t = this.peek();
        if (t.type === 'str' || t.type === 'name' || t.type === 'num') {
            this.next();
            return t.value;
        }
...
function describe(t, value) {
    ...
    if (t === 'newline')
        return 'a new line';
    if (t === 'name')
        return value ? `"${value}"` : 'a name';
    if (t === 'num' || t === 'str')
        return value !== undefined ? `"${value}"` : t;
    return `"${t}"`;
}
