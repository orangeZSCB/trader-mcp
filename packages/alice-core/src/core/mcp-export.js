...
export function toMcpContent(result) {
    if (result != null &&
        ...
        if ('details' in result && result.details != null) {
            blocks.push({ type: 'text', text: JSON.stringify(result.details) });
        }
 ...
 */
function coerceIfNumber(schema) {
    ...
    if (def.type === 'number' && !def.coerce) {
        ...
        if (def.checks?.length > 0)
            coerced = coerced.with(...def.checks);
        return coerced;
    }
    // z.number().optional()
    if (def.type === 'optional' && def.innerType?._zod?.def?.type === 'number' && !def.innerType._zod.def.coerce) {
        ...
        if (innerChecks?.length > 0)
            coerced = coerced.with(...innerChecks);
 ...
 */
export function extractMcpShape(tool) {
    const rawShape = tool.inputSchema?.shape ?? {};
    const coerced = {};
 ...
 */
export function wrapToolExecute(tool) {
    return async (args) => {
        try {
...
//# sourceMappingURL=mcp-export.js.map