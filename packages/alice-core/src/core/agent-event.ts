...
// CronFirePayload — inlined for the trading-mcp extraction. Original lives
// at `src/task/cron/engine.ts` in OpenAlice. UTA itself never runs cron, so
// we keep only the type shape for event-log validation, not the engine.
...
// CronFirePayload is defined above (see top of file) — UTA-only extraction
// inlines the type instead of pulling from `task/cron/engine.js`.
