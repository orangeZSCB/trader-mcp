...
function globalProviderKeysFile() {
    const root = process.env['OPENALICE_GLOBAL_DIR'] ?? join(homedir(), '.openalice');
    return join(root, 'provider-keys.json');
}
async function readGlobalProviderKeys() {
    try {
        const raw = JSON.parse(await readFile(globalProviderKeysFile(), 'utf-8'));
        if (typeof raw !== 'object' || raw === null)
            return {};
        return Object.fromEntries(Object.entries(raw).filter(([, v]) => typeof v === 'string' && v !== ''));
    }
    catch {
        return {};
    }
...
async function applyGlobalProviderKeys(parsed) {
...
async function mirrorProviderKeysToGlobal(keys) {
    const global = await readGlobalProviderKeys();
 ...
 */
export function credentialWires(cred) {
    if (cred.wires && Object.keys(cred.wires).length > 0)
        return cred.wires;
    if (cred.wireShape)
        return { [cred.wireShape]: cred.baseUrl ?? '' };
    return {};
}
...
async function removeJsonFile(filename) {
    try {
...
async function parseAndSeed(filename, schema, raw) {
...
export async function loadConfig() {
    ...
    await runMigrations();
    const files = ['engine.json', 'agent.json', 'crypto.json', 'securities.json', 'market-data.json', 'compaction.json', 'ai-provider-manager.json', 'snapshot.json', 'mcp.json', 'connectors.json', 'news.json', 'tools.json', 'webhook.json', 'trading.json'];
    ...
    if (envWebPort !== null)
        config.connectors.web.port = envWebPort;
    const envMcpPort = parseEnvPort(process.env['OPENALICE_MCP_PORT']);
    if (envMcpPort !== null)
        config.mcp.port = envMcpPort;
    return config;
}
 ...
 */
function migrateLegacyUTA(raw) {
    const id = String(raw['id'] ?? '');
    const label = raw['label'];
    const enabled = raw['enabled'];
    const guards = raw['guards'];
    const type = String(raw['type'] ?? '');
    const bc = (raw['brokerConfig'] ?? {});
    const base = (presetId, presetConfig) => ({
        id,
    ...
    if (type === 'ccxt') {
        ...
        const apiKey = bc['apiKey'];
        // Legacy used both `secret` and `apiSecret` (alias); new presets use `secret`.
        const secret = (bc['secret'] ?? bc['apiSecret']);
        const password = bc['password'];
        const sandbox = Boolean(bc['sandbox']);
        const demoTrading = Boolean(bc['demoTrading']);
        const walletAddress = bc['walletAddress'];
        const privateKey = bc['privateKey'];
        switch (exchange) {
            case 'okx':
 ...
 */
async function writeAccountsFile(validated) {
    await mkdir(CONFIG_DIR, { recursive: true });
...
export async function readUTAsConfig() {
    ...
    if (isSealedEnvelope(raw)) {
        try {
            ...
            console.error(`accounts.json could not be unsealed: ${err instanceof Error ? err.message : String(err)}\n` +
    ...
    if (Array.isArray(raw) && raw.some((r) => isLegacyRecord(r))) {
        ...
        console.warn(`accounts.json: migrated ${migrated.length - skipped.length} legacy record(s) to preset shape ` +
            `(backup: ${backupPath}).` +
...
export async function writeUTAsConfig(utas) {
 ...
 */
export async function wipeUTATradingData(id) {
 ...
 * normal init).
 */
export async function purgeEphemeralUTAs(utas) {
...
export async function resolveCredential(slug) {
    const config = await readAIProviderConfig();
    const cred = config.credentials[slug];
    if (!cred)
        throw new Error(`Unknown credential: "${slug}"`);
    return { ...cred };
}
...
export async function writeCredential(slug, credential) {
    const config = await readAIProviderConfig();
...
export async function addCredential(credential) {
    const config = await readAIProviderConfig();
    const validated = credentialSchema.parse(credential);
    const match = Object.entries(config.credentials).find(([, c]) => c.vendor === validated.vendor &&
    ...
    let n = 1;
    while (taken.has(`${validated.vendor}-${n}`))
        n++;
...
export async function deleteCredential(slug) {
    const config = await readAIProviderConfig();
...
export const validSections = Object.keys(sectionSchemas);
/** Validate and write a config section to disk. Returns the validated config. */
export async function writeConfigSection(section, data) {
    ...
    if (section === 'marketData') {
        const keys = validated.providerKeys;
        if (keys)
            await mirrorProviderKeysToGlobal(keys);
    }
    return validated;
}
/** Read web sub-channel definitions from disk. Returns empty array if file missing. */
export async function readWebSubchannels() {
...
export async function writeWebSubchannels(channels) {
