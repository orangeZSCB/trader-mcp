...
import { seal, unseal, isSealedEnvelope, type SealedEnvelope } from '@openalice-trading/alice-core/core/sealing.js'
...
async function readAccounts(): Promise<AccountConfig[]> {
  ...
  try {
    ...
    if (isSealedEnvelope(parsed)) {
      const accounts = await unseal<AccountConfig[]>(parsed as SealedEnvelope)
      return accounts
    }
