...
import { tool } from 'ai';
import { z } from 'zod';
import { fetchSectorRotation } from '@openalice-trading/alice-core/domain/analysis/sector-rotation.js';
import { createHubFetcher, markLocal } from '@openalice-trading/alice-core/domain/market-data/reference/hub.js';
export function createSectorRotationTools(equityClient, hub) {
...
exact definitions and the fund-flow-proxy caveat.`,
            ...
            execute: async () => {
                const fromHub = await viaHub('rotation');
                if (fromHub)
                    return fromHub;
