...
import { tool } from 'ai';
import { z } from 'zod';
export function createEquityTools(equityClient) {
    return {
...
If unsure about the symbol, use marketSearchForResearch to find it.`,
            ...
            execute: async ({ symbol, type, period, limit }) => {
                const params = { symbol, provider: 'yfinance' };
                if (period)
...
If unsure about the symbol, use marketSearchForResearch to find it.`,
            ...
            execute: async ({ symbol, period, limit, ttm }) => {
                ...
                const params = { symbol, provider: 'fmp', ttm: ttm ?? 'include' };
                if (period)
                    params.period = period;
                if (limit)
                    params.limit = limit;
...
populated on the default yfinance data only.`,
            ...
            execute: async ({ type, sortBy }) => {
                ...
                if (sortBy === 'relative_volume' || sortBy === 'dollar_volume') {
                    ...
                    rows = [...rows].sort((a, b) => (b[sortBy] ?? -Infinity) - (a[sortBy] ?? -Infinity));
                }
                return rows;
            },
        }),
