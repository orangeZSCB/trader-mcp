...
export function val(v) {
    ...
    const s = v instanceof Decimal ? v.toFixed()
        : typeof v === 'object' && 'toFixed' in v ? v.toFixed()
            : String(v);
    if (s === '' || s === UNSET_DOUBLE_STR || s === UNSET_I32_STR)
        return undefined;
    if (UNSET_DECIMAL_RE.test(s))
        return undefined;
    return s;
}
...
export function compactContract(c) {
...
export function compactOrderFields(o) {
    ...
    if (k['outsideRth'] === true)
        out['outsideRth'] = true;
    const filled = val(k['filledQuantity']);
    if (filled)
        out['filledQuantity'] = filled;
    return out;
}
/** Operation (staged / committed) → human-scale summary. */
export function compactOperation(op) {
...
export function compactStageResult(r) {
    if (!r || typeof r !== 'object')
        return {};
    const k = r;
    return {
...
export function compactPushResult(r) {
    if (!r || typeof r !== 'object')
        return {};
    const k = r;
    return {
...
export function compactContractDetails(details) {
    ...
    for (const [key, v] of Object.entries(k)) {
        if (key === 'contract') {
            out['contract'] = compactContract(v);
            continue;
        }
        if (v == null || typeof v === 'object')
            continue;
        const s = val(v);
        if (s !== undefined && s !== '0' && s !== 'false')
            out[key] = v;
    }
    return out;
}
/** AccountInfo → 2dp money (display precision; the ledger keeps full). */
export function compactAccountInfo(info) {
    if (!info || typeof info !== 'object')
        return {};
    const k = info;
    const out = { baseCurrency: k['baseCurrency'] };
    ...
    if (k['dayTradesRemaining'] != null)
        out['dayTradesRemaining'] = k['dayTradesRemaining'];
    return out;
}
