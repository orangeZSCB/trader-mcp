...
import { compactAccountInfo, compactCommit, compactContract, compactContractDetails, compactOperation, compactOrderFields, compactPushResult, compactStageResult, compactStatus, money, price, } from './compact.js';
...
function handleBrokerError(err) {
 ...
 */
function summarizeOrder(o, source, stringOrderId) {
    const order = o.order;
...
const sourceDesc = (required, extra) => {
...
async function noAccountsError(manager, source) {
    try {
        const ids = (await manager.listUTAs()).map((u) => u.id);
...
async function stageAndMaybeCommit(uta, commitMessage) {
    const staged = compactStageResult(await uta.stage());
    if (!commitMessage)
        return staged;
    const committed = await uta.commit(commitMessage);
    return {
        ...staged,
        committed: { hash: committed['hash'], message: committed['message'] },
        nextStep: 'Show this commit to the user via tradingStatus, ask for explicit approval, then call tradingPush. See the openalice-trading skill for the full ritual.',
    };
}
export function createTradingTools(manager) {
    return {
        listUTAs: tool({
...
hitting the broker, which otherwise expects the bare base ticker.`,
            ...
            execute: async ({ pattern, assetClass, source }) => {
                ...
                if (!brokerPattern)
                    return { results: [], message: 'Empty pattern.' };
                ...
                const all = [];
                const settled = await Promise.allSettled(targets.map(async (uta) => ({ id: uta.id, results: await uta.searchContracts(brokerPattern) })));
                for (const r of settled) {
                    if (r.status !== 'fulfilled')
                        continue;
                    for (const desc of r.value.results) {
                        const contract = compactContract(desc.contract);
                        ...
                        const expandable = typeof contract['aliceId'] === 'string' && contract['aliceId'].includes('|issuer:');
                        all.push({ source: r.value.id, ...desc, contract, ...(expandable ? { expandable: true } : {}) });
                    }
                }
                if (all.length === 0)
                    return { results: [], message: `No contracts found matching "${brokerPattern}" (input: "${pattern}").` };
                return all;
            },
        }),
        getContractDetails: tool({
            ...
            execute: async ({ source, symbol, aliceId, secType, currency }) => {
                ...
                if (symbol)
                    query.symbol = symbol;
                if (secType)
                    query.secType = coerceSecType(secType);
                if (currency)
...
If this tool returns an error with transient=true, wait a few seconds and retry once before reporting to the user.`,
            ...
            execute: async ({ source, symbol }) => {
                const targets = await manager.resolve(source);
                if (targets.length === 0)
                    return { positions: [], ...(await noAccountsError(manager, source)) };
                ...
                try {
                    ...
                    for (const uta of targets) {
                        ...
                        let idx = 0;
                        for (const pos of positions) {
                            if (symbol && symbol !== 'all' && pos.contract.symbol !== symbol) {
                                idx++;
                                continue;
                            }
                    ...
                    if (allWarnings.length > 0)
                        return { positions: allPositions, fxWarnings: allWarnings };
                    return allPositions;
                }
                catch (err) {
                    return handleBrokerError(err);
                ...
                try {
                    ...
                    if (groupBy === 'contract') {
                        ...
                        for (const s of summaries) {
                            ...
                            if (!grouped[key])
                                grouped[key] = { symbol: s.symbol, orders: [] };
...
IMPORTANT: Check this BEFORE making new trading decisions.`,
            ...
            execute: async ({ source, limit, symbol }) => {
                ...
                const allEntries = [];
                for (const uta of targets) {
                    for (const entry of await uta.log({ limit, symbol }))
                        allEntries.push({ source: uta.id, ...entry });
                }
        ...
        tradingShow: tool({
            ...
            execute: async ({ hash }) => {
                for (const uta of await manager.resolve()) {
                    ...
                    if (commit)
                        return { source: uta.id, ...compactCommit(commit) };
                }
        ...
        simulatePriceChange: tool({
            ...
            execute: async ({ source, priceChanges }) => {
                ...
                const results = [];
                for (const uta of targets)
                    results.push({ source: uta.id, ...await uta.simulatePriceChange(priceChanges) });
                return results.length === 1 ? results[0] : results;
            },
        }),
        // ==================== Mutations ====================
...
Optional: attach takeProfit and/or stopLoss for automatic exit orders.`,
        ...
        tradingCommit: tool({
            ...
            execute: async ({ source, message }) => {
                ...
                if (results.length === 0)
                    return { message: 'No staged operations to commit.' };
                return results.length === 1 ? results[0] : results;
            },
        }),
        tradingPush: tool({
            description: `Execute the pending commit against the live broker.

This is the IRREVERSIBLE step in the Trade-as-Git workflow. The
calling agent MUST have shown the user the pending commit (via
tradingStatus) and received explicit user approval BEFORE invoking
this tool — see the openalice-trading skill for the required ritual.

Behaviour:
- If a pending commit exists on the targeted account(s), pushes it
  and returns the broker submission result.
- If staged operations are not yet committed, returns an error and
  does NOT push (the caller must commit first).
- If nothing is committed and nothing is staged, returns a no-op.

Returns the per-account push result with the broker's response for
each operation (orderId, status, errors). Surface partial failures
to the user verbatim — do not paper over rejected legs.`,
            inputSchema: z.object({
                source: z.string().optional().describe(sourceDesc(false, 'If omitted, pushes all accounts with pending commits.')),
            }).meta({ examples: [{ source: 'alpaca-paper' }] }),
            execute: async ({ source }) => {
                const targets = await manager.resolve(source);
                if (targets.length === 0)
                ...
                const uncommitted = statuses.filter(({ status }) => status.staged.length > 0 && !status.pendingMessage);
                ...
                const withPending = statuses.filter(({ status }) => status.pendingMessage);
                if (withPending.length === 0) {
                    return { message: 'No committed operations to push.' };
                }
                const pushed = [];
                const errors = [];
                for (const { uta } of withPending) {
                    try {
                        const result = await uta.push();
                        pushed.push({ source: uta.id, ...compactPushResult(result) });
                    }
                    catch (err) {
                        errors.push({ source: uta.id, error: err instanceof Error ? err.message : String(err) });
                    }
                }
                if (pushed.length === 0 && errors.length > 0) {
                    return { error: 'All pushes failed.', errors };
                }
                return {
                    message: errors.length === 0 ? 'Pushed successfully.' : 'Pushed with some failures — review per-account results below.',
                    pushed: pushed.length === 1 ? pushed[0] : pushed,
        ...
        tradingReject: tool({
            ...
            execute: async ({ source, reason }) => {
                try {
                    ...
                    const status = await uta.status();
                    if (status.staged.length === 0)
                        return { message: 'Nothing staged to reject.' };
                    ...
                    if (!status.pendingHash)
                        await uta.commit(reason ?? 'discarding staged operations');
        ...
        orderHistory: tool({
            ...
            execute: async ({ source, limit }) => {
                ...
                if (targets.length === 0)
                    return await noAccountsError(manager, source);
                try {
                    const all = (await Promise.all(targets.map(async (uta) => (await uta.orderHistory(limit ?? 50)).map((o) => ({ account: uta.id, ...o }))))).flat();
        ...
        tradeHistory: tool({
            ...
            execute: async ({ source, limit }) => {
                ...
                if (targets.length === 0)
                    return await noAccountsError(manager, source);
                try {
                    const all = (await Promise.all(targets.map(async (uta) => (await uta.tradeHistory(limit ?? 50)).map((t) => ({ account: uta.id, ...t }))))).flat();
            ...
            execute: async ({ source, delayMs }) => {
                ...
                for (const uta of targets) {
                    ...
                    if (result.updatedCount > 0)
                        results.push({ source: uta.id, ...result });
                }
                if (results.length === 0)
