...
import { GdpRealDataSchema } from '../../../standard-models/gdp-real.js';
...
export const OECDGdpRealQueryParamsSchema = z.object({
    country: z.string().default('united_states'),
    start_date: z.string().nullable().default(null),
    end_date: z.string().nullable().default(null),
    frequency: z.enum(['annual', 'quarter']).default('annual'),
}).passthrough();
export class OECDGdpRealFetcher extends Fetcher {
    static requireCredentials = false;
    static transformQuery(params) {
        return OECDGdpRealQueryParamsSchema.parse(params);
    }
    static async extractData(query, _credentials) {
        ...
        const rows = await fetchOecdCsv('OECD.SDD.NAD,DSD_NAMAIN1@DF_TABLE1_EXPENDITURE_HCPC,1.0', `${cc}.${freq}.S1.S1.B1GQ._Z._Z._Z.L.N.`);
        return rows
    ...
    static transformData(query, data) {
        ...
        return filterAndSort(data, query.start_date, query.end_date)
            .map(d => GdpRealDataSchema.parse(d));
    }
}
//# sourceMappingURL=gdp-real.js.map