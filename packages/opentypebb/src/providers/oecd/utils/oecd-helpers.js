...
export const CODE_TO_NAME = Object.fromEntries(Object.entries(COUNTRY_MAP).map(([k, v]) => [v, k.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())]));
export const FREQ_MAP = { annual: 'A', quarter: 'Q', monthly: 'M' };
/**
 ...
 */
export async function fetchOecdCsv(dataflow, dimensions) {
    const url = `https://sdmx.oecd.org/public/rest/data/${dataflow}` +
        `/${dimensions}` +
 ...
 */
export function resolveCountryCodes(country) {
    return country
        .split(',')
 ...
 */
export function filterAndSort(data, startDate, endDate) {
    let filtered = data;
    if (startDate)
        filtered = filtered.filter(d => String(d.date) >= startDate);
    if (endDate)
        filtered = filtered.filter(d => String(d.date) <= endDate);
    return filtered.sort((a, b) => String(a.date).localeCompare(String(b.date)));
}
//# sourceMappingURL=oecd-helpers.js.map