...
export class BLSBlsSearchFetcher extends Fetcher {
    ...
    static async extractData(query, _credentials) {
        ...
        const results = COMMON_SERIES.filter(s => s.title.toLowerCase().includes(q) ||
            s.series_id.toLowerCase().includes(q) ||
            s.survey_abbreviation.toLowerCase().includes(q)).slice(0, query.limit);
        if (results.length === 0)
            throw new EmptyDataError(`No BLS series matching "${query.query}" found.`);
        return results;
    }
