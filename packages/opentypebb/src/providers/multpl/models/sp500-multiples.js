 ...
 */
function parseValue(valueStr, isPercent) {
...
export class MultplSP500MultiplesFetcher extends Fetcher {
    ...
    static async extractData(query, _credentials) {
        ...
        const tasks = series.map(async (seriesName) => {
            ...
            try {
                ...
                for (const row of rows) {
                    ...
                    if (query.start_date && date < query.start_date)
                        continue;
                    if (query.end_date && date > query.end_date)
                        continue;
        ...
        if (results.length === 0)
            throw new EmptyDataError('No data found.');
        return results;
    }
...
//# sourceMappingURL=sp500-multiples.js.map