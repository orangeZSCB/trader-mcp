...
function inferAssetType(d) {
    ...
    if (desc.includes('treasury') || desc.includes('bill'))
        return 'Treasury';
    if (desc.includes('%') || desc.includes('due') || desc.includes('pct'))
        return 'Bond';
...
export class FMPGovernmentTradesFetcher extends Fetcher {
    ...
    static async extractData(query, credentials) {
        ...
        if (query.symbol) {
            ...
            const settled = await Promise.allSettled(urls.map(async ({ url, chamber }) => {
            ...
            for (const ep of endpoints) {
                ...
                try {
                    ...
                    const data = await amakeRequest(`${baseUrl}${latestEp}?page=0&limit=${Math.min(limit, 250)}&apikey=${apiKey}`);
                    if (data?.length) {
...
//# sourceMappingURL=government-trades.js.map