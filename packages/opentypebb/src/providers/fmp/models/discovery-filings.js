...
export class FMPDiscoveryFilingsFetcher extends Fetcher {
    ...
    static async extractData(query, credentials) {
        ...
        qs.set('to', endDate);
        if (query.form_type)
            qs.set('formType', query.form_type);
        ...
        for (let page = 0; page < pages; page++) {
            try {
                const data = await getDataMany(`${baseUrl}?${qs.toString()}&page=${page}&limit=1000`);
        ...
        return allResults.sort((a, b) => String(b.acceptedDate ?? '').localeCompare(String(a.acceptedDate ?? '')));
    }
...
//# sourceMappingURL=discovery-filings.js.map