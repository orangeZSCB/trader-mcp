 ...
 */
function generateUrls(startDate, endDate, apiKey) {
    const urls = [];
    ...
    while (current <= end) {
        ...
        urls.push(`https://financialmodelingprep.com/stable/treasury-rates?from=${fromStr}&to=${toStr}&apikey=${apiKey}`);
        current = next;
    }
    return urls;
}
export class FMPTreasuryRatesFetcher extends Fetcher {
    ...
    static async extractData(query, credentials) {
        ...
        const chunks = await getDataUrls(urls);
...
//# sourceMappingURL=treasury-rates.js.map