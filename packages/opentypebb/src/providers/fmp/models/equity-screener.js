...
export class FMPEquityScreenerFetcher extends Fetcher {
    ...
    static async extractData(query, credentials) {
        ...
        for (const [apiName, , val] of mappings) {
            if (val != null)
                qs.set(apiName, String(val));
        }
        if (query.sector)
            qs.set('sector', query.sector);
        if (query.industry)
            qs.set('industry', query.industry);
        if (query.country)
            qs.set('country', query.country);
        if (query.exchange)
            qs.set('exchange', query.exchange);
        if (query.is_etf != null)
            qs.set('isEtf', String(query.is_etf));
        if (query.is_active != null)
            qs.set('isActivelyTrading', String(query.is_active));
        if (query.is_fund != null)
            qs.set('isFund', String(query.is_fund));
        if (query.all_share_classes != null)
            qs.set('includeAllShareClasses', String(query.all_share_classes));
        if (query.limit)
            qs.set('limit', String(query.limit));
        return getDataMany(`https://financialmodelingprep.com/stable/company-screener?${qs.toString()}`);
    }
