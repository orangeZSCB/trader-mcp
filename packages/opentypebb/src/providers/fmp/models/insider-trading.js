...
export class FMPInsiderTradingFetcher extends Fetcher {
    ...
    static async extractData(query, credentials) {
        ...
        const urls = Array.from({ length: pages }, (_, page) => `${baseUrl}?${queryStr}&page=${page}&limit=${limit}&apikey=${apiKey}`);
        const results = await getDataUrls(urls);
        return results.flat();
    }
    static transformData(query, data) {
        const sorted = query.statistics
            ...
            : [...data].sort((a, b) => String(b.filingDate ?? '').localeCompare(String(a.filingDate ?? '')));
        return sorted.map((d) => {
