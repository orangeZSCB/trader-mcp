...
export class FMPCurrencySnapshotsFetcher extends Fetcher {
    ...
    static async extractData(_query, credentials) {
        ...
        return getDataMany(`https://financialmodelingprep.com/stable/batch-forex-quotes?short=false&apikey=${apiKey}`);
    }
    static transformData(query, data) {
        ...
        for (const base of bases) {
            for (const d of data) {
                ...
                if (counterCurrencies && !counterCurrencies.includes(counterCurrency))
                    continue;
...
//# sourceMappingURL=currency-snapshots.js.map