 ...
 */
export async function responseCallback(response) {
    ...
    if (data && typeof data === 'object' && !Array.isArray(data)) {
        ...
        if (errorMessage != null) {
            ...
            const isUnauthorized = msg.includes('upgrade') ||
            ...
            throw new OpenBBError(`FMP Error Message -> Status code: ${response.status} -> ${errorMessage}`);
        }
    }
 ...
 */
export async function getDataUrls(urls) {
    const results = await Promise.all(urls.map((url) => amakeRequest(url, { responseCallback })));
    return results;
}
/**
 ...
 */
export async function getDataMany(url, subDict) {
 ...
 */
export async function getDataOne(url) {
    let data = await getData(url);
 ...
 */
export function createUrl(version, endpoint, apiKey, query, exclude) {
    const params = { ...(query ?? {}) };
...
export function mostRecentQuarter(base) {
    ...
    if (month < 7)
        return new Date(d.getFullYear(), 2, 31);
    if (month < 10)
        return new Date(d.getFullYear(), 5, 30);
 ...
 */
export function getQueryString(params, exclude = []) {
    const excludeSet = new Set(exclude);
    const searchParams = new URLSearchParams();
    for (const [key, value] of Object.entries(params)) {
        if (!excludeSet.has(key) && value !== null && value !== undefined) {
 ...
 */
export async function getHistoricalOhlc(query, credentials) {
    ...
    const getOne = async (symbol) => {
        ...
        try {
            const response = await amakeRequest(url, { responseCallback });
            if (typeof response === 'object' && response !== null && !Array.isArray(response)) {
                ...
                const historical = dict['historical'];
    ...
    if (results.length === 0) {
        throw new EmptyDataError(messages.length > 0 ? messages.join(' ') : 'No data found');
    }
    return results;
}
