...
async function arcgisQuery(baseUrl, opts) {
    ...
    if (opts.orderBy)
        qs.set('orderByFields', opts.orderBy);
    if (opts.limit)
        qs.set('resultRecordCount', String(opts.limit));
    const data = await amakeRequest(`${baseUrl}?${qs.toString()}`);
    if (data.error)
        throw new Error(`PortWatch ArcGIS error: ${data.error.message ?? 'unknown'}`);
...
function nameOrIdWhere(value) {
    return `(portid = ${sq(value)} OR LOWER(portname) LIKE ${sq('%' + value.toLowerCase() + '%')})`;
}
function dateRangeWhere(start, end) {
    const parts = [];
    if (start)
        parts.push(`date >= DATE ${sq(start)}`);
    if (end)
        parts.push(`date <= DATE ${sq(end)}`);
    return parts;
}
// --- Port Info ---
export class ImfPortInfoFetcher extends Fetcher {
    ...
    static async extractData(query) {
        ...
        if (rows.length === 0)
            throw new EmptyDataError(`No PortWatch port matches "${query.port}".`);
        return rows;
    }
...
export class ImfPortVolumeFetcher extends Fetcher {
    ...
    static async extractData(query) {
        ...
        if (rows.length === 0)
            throw new EmptyDataError(`No PortWatch activity for "${query.port}" in range.`);
        return rows;
    }
    static transformData(_query, data) {
        ...
        return data.map((d) => ({
            ...
            volume: typeof d.import === 'number' && typeof d.export === 'number'
                ? d.import + d.export
...
export class ImfChokepointInfoFetcher extends Fetcher {
    ...
    static async extractData(query) {
        ...
        if (rows.length === 0)
            throw new EmptyDataError(`No PortWatch chokepoint matches "${query.chokepoint}".`);
        return rows;
    }
    static transformData(_query, data) {
        return data.map((d) => ({
...
export class ImfChokepointVolumeFetcher extends Fetcher {
    ...
    static async extractData(query) {
        ...
        if (query.chokepoint)
            parts.unshift(nameOrIdWhere(query.chokepoint));
        ...
        if (rows.length === 0)
            throw new EmptyDataError(`No PortWatch transit data for "${query.chokepoint}" in range.`);
        return rows;
    }
    static transformData(_query, data) {
        return data.map((d) => ({
...
//# sourceMappingURL=port-watch.js.map