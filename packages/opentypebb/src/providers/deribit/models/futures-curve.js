...
export class DeribitFuturesCurveFetcher extends Fetcher {
    ...
    static async extractData(query, _credentials) {
        ...
        if (instrumentNames.length === 0)
            throw new EmptyDataError('No instruments found.');
        const results = [];
        ...
        if (results.length === 0)
            throw new EmptyDataError('No ticker data found.');
        return results;
    }
    static transformData(_query, data) {
        ...
        return data.map(d => {
            ...
            const price = d.last_price ?? d.mark_price ?? 0;
...
export function deribitExpiryToIso(raw) {
