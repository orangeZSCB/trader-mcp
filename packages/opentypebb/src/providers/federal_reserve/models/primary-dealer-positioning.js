...
export class FedPrimaryDealerPositioningFetcher extends Fetcher {
    ...
    static async extractData(query) {
        const byDate = {};
        let firstError = null;
        await Promise.all(Object.entries(SERIES).map(async ([keyid, field]) => {
            try {
                const data = await amakeRequest(`${NYFED_PD_BASE}/${keyid}.json`);
                for (const row of data.pd?.timeseries ?? []) {
                    const v = parseFloat(row.value);
                    if (Number.isNaN(v))
                        continue;
                    if (query.start_date && row.asofdate < query.start_date)
                        continue;
                    if (query.end_date && row.asofdate > query.end_date)
                        continue;
