...
import { getFuturesSymbols, getHistoricalData } from '../utils/helpers.js';
...
function getExpirationMonth(symbol) {
...
function generateFuturesSymbols(baseSymbol, exchange, numMonths = 36) {
    const symbols = [];
    const now = new Date();
...
export class YFinanceFuturesCurveFetcher extends Fetcher {
    ...
    static async extractData(query, _credentials) {
        ...
        const results = await Promise.allSettled(chainSymbols.map(async (sym) => {
            try {
        ...
        for (const r of results) {
            if (r.status === 'fulfilled' && r.value) {
                ...
                if (consecutiveEmpty >= 12 && curve.length > 0)
                    break;
            }
        }
        if (!curve.length)
            throw new EmptyDataError(`No futures curve data for ${query.symbol}`);
        // Sort by expiration
...
//# sourceMappingURL=futures-curve.js.map