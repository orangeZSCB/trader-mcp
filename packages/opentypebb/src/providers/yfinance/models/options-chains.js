...
async function fetchViaYF2(symbol) {
    ...
    const allContracts = [];
    const processOptions = (options, type, expirationStr) => {
    ...
    for (let i = 0; i < remainingDates.length; i += batchSize) {
        ...
        await Promise.allSettled(batch.map(async (expDate) => {
...
async function fetchViaDirect(symbol) {
    ...
    if (!resp.ok)
        throw new EmptyDataError(`Yahoo options API returned ${resp.status}`);
    const json = await resp.json();
    const result = json?.optionChain?.result?.[0];
    if (!result)
        throw new EmptyDataError(`No options data for ${symbol}`);
    const expirationEpochs = result.expirationDates ?? [];
    if (!expirationEpochs.length)
        throw new EmptyDataError(`No option expirations for ${symbol}`);
    const underlyingPrice = result.quote?.regularMarketPrice ?? null;
    const today = new Date().toISOString().slice(0, 10);
    const allContracts = [];
    const processChain = (options, type, expirationStr) => {
        for (const opt of options) {
            const strike = opt.strike ?? 0;
            const now = new Date();
            const exp = new Date(expirationStr);
    ...
    for (let i = 0; i < remaining.length; i += batchSize) {
        ...
        await Promise.allSettled(batch.map(async (epoch) => {
