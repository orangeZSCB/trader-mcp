...
export class YFinanceEquityProfileFetcher extends Fetcher {
    ...
    static async extractData(query, credentials) {
        ...
        const results = await Promise.allSettled(symbols.map(s => getQuoteSummary(s, ['summaryProfile', 'summaryDetail', 'price', 'defaultKeyStatistics'])));
        const data = [];
    ...
    static transformData(query, data) {
        if (!data.length)
            throw new EmptyDataError('No profile data returned');
        return data.map(d => {
...
//# sourceMappingURL=equity-profile.js.map