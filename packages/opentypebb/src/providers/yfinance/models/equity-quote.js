...
export class YFinanceEquityQuoteFetcher extends Fetcher {
    ...
    static async extractData(query, credentials) {
        ...
        const results = await Promise.allSettled(symbols.map(s => getQuoteSummary(s, ['price', 'summaryDetail', 'defaultKeyStatistics'])));
        const data = [];
    ...
    static transformData(query, data) {
        if (!data.length)
            throw new EmptyDataError('No quote data returned');
        return data.map(d => {
            const aliased = applyAliases(d, ALIAS_DICT);
            // yahoo-finance2 returns bidSize/askSize in lots (hundreds), normalize to board lots
            if (typeof aliased.bid_size === 'number')
                aliased.bid_size = Math.round(aliased.bid_size / 100);
            if (typeof aliased.ask_size === 'number')
                aliased.ask_size = Math.round(aliased.ask_size / 100);
            return YFinanceEquityQuoteDataSchema.parse(aliased);
        });
    }
}
//# sourceMappingURL=equity-quote.js.map