...
// ==================== Helpers ====================
function makeContract(aliceId) {
    const c = new Contract();
    c.aliceId = aliceId;
    c.symbol = aliceId.split('|').pop() ?? aliceId;
    return c;
}
function buy(aliceId, qty, price) {
...
function sell(aliceId, qty, price) {
    const order = new Order();
    order.action = 'SELL';
    order.totalQuantity = new Decimal(qty);
    return {
...
function reconcile(aliceId, delta, mark) {
    return {
        op: {
...
describe('recomputeCostBasisFromCommits', () => {
    ...
    it('rejects unsuccessful results — does not affect cost basis', () => {
        ...
        const result = recomputeCostBasisFromCommits([commit(failedBuy, goodBuy)], ALICE_ID);
    ...
    it('handles closePosition op as a sell', () => {
        ...
        const result = recomputeCostBasisFromCommits([commit(buyFill), commit(closeOp)], ALICE_ID);
        expect(result.avgCost.toNumber()).toBe(0);
        expect(result.qty.toNumber()).toBe(0);
    });
...
function pendingBuy(aliceId, qty, orderId) {
    const order = new Order();
    order.action = 'BUY';
    order.totalQuantity = new Decimal(qty);
    return {
...
describe('recomputeCostBasisFromCommits — sync fills', () => {
    ...
    it('ignores sync fills for other aliceIds and non-filled transitions', () => {
        const commits = [
            ...
            syncCommit({ orderId: 'ord-other', status: 'filled', qty: 1, price: 2000 }, { orderId: 'ord-cancelled', status: 'cancelled' }),
        ];
    ...
    it('attributes a sync fill of an OBSERVED external order at execution price', () => {
        ...
        const commits = [
            {
                ...commit(),
                operations: [{ action: 'observeExternalOrder', contract: makeContract(ALICE_ID), order }],
                results: [{ action: 'observeExternalOrder', success: true, orderId: 'ext-1', status: 'submitted' }],
            },
...
//# sourceMappingURL=cost-basis.spec.js.map