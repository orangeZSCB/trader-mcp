...
import { createSnapshotScheduler } from './scheduler.js';
...
function makeSubmittedOrder(symbol = 'AAPL') {
...
function makeFilledOrder(symbol = 'AAPL') {
...
describe('Snapshot Builder', () => {
    ...
    it('passes trigger field correctly', async () => {
        for (const trigger of ['scheduled', 'post-push', 'post-reject', 'manual']) {
...
describe('Snapshot Service', () => {
    ...
    it('takeAllSnapshots captures all accounts', async () => {
        ...
        const ids = events.map(e => e.payload.accountId);
    ...
    it('takeAllSnapshots: single failure does not affect others', async () => {
        ...
        expect(taken[0].payload.accountId).toBe('acc1');
