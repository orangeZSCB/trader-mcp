...
import { UNSET_DECIMAL, UNSET_DOUBLE } from '@traderalice/ibkr';
function toHistoryContract(contract) {
    if (!contract)
        return {};
...
export function projectOrderHistory(commits, opts = {}) {
    const byOrderId = new Map();
    const anonymous = []; // rejected-before-submit rows have no orderId
    for (const commit of commits) {
            ...
            if (op.action === 'placeOrder' || op.action === 'observeExternalOrder' || op.action === 'closePosition') {
                const entry = {
                    ...
                    ...(op.action === 'closePosition'
                        ? { side: 'SELL', orderType: 'MKT', ...(op.quantity != null && { quantity: String(op.quantity) }) }
                ...
                if (result?.orderId)
                    byOrderId.set(result.orderId, entry);
                else
        ...
        if (commit.operations.some((op) => op.action === 'syncOrders')) {
            for (const result of commit.results) {
                ...
                target.resolvedAt = commit.timestamp;
                if (result.filledQty)
                    target.filledQty = result.filledQty;
                if (result.filledPrice)
                    target.avgFillPrice = result.filledPrice;
            }
        }
    }
...
export function projectTradeHistory(commits, opts = {}) {
    ...
    const trades = [];
    const counted = new Set(); // orderId fills already recorded (origin vs sync)
    const push = (params) => {
    ...
    for (const commit of commits) {
        ...
        if (isSync) {
            for (const result of commit.results) {
                if (!result.orderId || result.status !== 'filled')
                    continue;
                if (!result.filledQty || !result.filledPrice || counted.has(result.orderId))
                    continue;
        ...
        for (let i = 0; i < commit.operations.length; i++) {
            ...
            if (op.action === 'reconcileBalance') {
                if (!result.filledQty || !result.filledPrice)
                    continue;
            ...
            if (op.action === 'placeOrder' || op.action === 'closePosition' || op.action === 'observeExternalOrder') {
                if (result.status !== 'filled' || !result.filledQty || !result.filledPrice)
                    continue;
                if (result.orderId && counted.has(result.orderId))
                    continue;
                push({
                    timestamp: commit.timestamp,
                    ...(result.orderId && { orderId: result.orderId }),
...
function commitSourceIsExternal(commits, orderId) {
    for (const commit of commits) {
        for (let i = 0; i < commit.operations.length; i++) {
