...
import { readUTAsConfig } from '@openalice-trading/alice-core/core/config.js';
...
export class UTAManager {
    ...
    resolveOne(source) {
        ...
        if (results.length > 1) {
            throw new Error(`Multiple UTAs match source "${source}": ${results.map((r) => r.id).join(', ')}. Use UTA id for exact match.`);
        }
        return results[0];
    }
    ...
    async searchContracts(pattern, accountId) {
        const targets = accountId
            ? [this.entries.get(accountId)].filter(Boolean)
            : Array.from(this.entries.values());
        const results = await Promise.all(targets.map(async (uta) => {
            if (uta.health !== 'healthy') {
                uta.nudgeRecovery();
                return { accountId: uta.id, results: [] };
            }
            try {
    ...
    async getContractDetails(query, accountId) {
    ...
    // ==================== Cleanup ====================
    async closeAll() {
        await Promise.allSettled(Array.from(this.entries.values()).map((uta) => uta.close()));
        this.entries.clear();
    }
}
//# sourceMappingURL=uta-manager.js.map