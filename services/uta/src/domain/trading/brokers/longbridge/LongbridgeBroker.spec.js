...
vi.mock('longbridge', () => {
    ...
    const OrderSide = { Unknown: 0, Buy: 1, Sell: 2 };
    const OrderType = {
        Unknown: 0, LO: 1, ELO: 2, MO: 3, AO: 4, ALO: 5, ODD: 6,
        LIT: 7, MIT: 8, TSLPAMT: 9, TSLPPCT: 10, TSMAMT: 11, TSMPCT: 12, SLO: 13,
    };
    const TimeInForceType = { Unknown: 0, Day: 1, GoodTilCanceled: 2, GoodTilDate: 3 };
    const Market = { Unknown: 0, US: 1, HK: 2, CN: 3, SG: 4, Crypto: 5 };
    return {
...
function attachMockContexts(broker) {
    const trade = {
        accountBalance: vi.fn(),
        stockPositions: vi.fn(),
        submitOrder: vi.fn(),
    ...
    broker.tradeCtx = trade;
    broker.quoteCtx = quote;
    return { trade, quote };
}
...
describe('LongbridgeBroker — init()', () => {
    ...
    it('resolves on successful accountBalance probe', async () => {
        ...
        lb.TradeContext.new.mockReturnValueOnce({
            accountBalance: vi.fn().mockResolvedValue([]),
        });
        lb.QuoteContext.new.mockReturnValueOnce({});
    ...
    it('with FxService, folds USD + CNY into HKD via cross-rate', async () => {
        ...
        b.setFxService({
            convertToUsd: async (amount, currency) => {
                const rates = { USD: 1, HKD: 1 / 7.8, CNY: 1 / 7.8 };
...
function mockBalance(currency, fields) {
    return {
        currency,
...
//# sourceMappingURL=LongbridgeBroker.spec.js.map