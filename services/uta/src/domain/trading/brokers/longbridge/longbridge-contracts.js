 ...
 */
import { ContractDescription, OrderState } from '@traderalice/ibkr';
 ...
 */
export function makeContract(lbSymbol) {
 ...
 */
export function parseLbSymbol(lbSymbol) {
    const idx = lbSymbol.lastIndexOf('.');
    if (idx < 0)
        return { ticker: lbSymbol.toUpperCase(), suffix: 'US' };
...
function inferSuffixFromCurrency(currency) {
 ...
 */
export function marketToSuffix(market) {
 ...
 */
export function mapLbOrderStatus(status) {
    switch (status) {
        case 5: // Filled
            return 'Filled';
        case 14: // Rejected
