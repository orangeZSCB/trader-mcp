...
export function makeOpenOrder(overrides = {}) {
    const contract = overrides.contract ?? makeContract();
...
export function makePlaceOrderResult(overrides = {}) {
    return {
        success: true,
        orderId: 'order-1',
        ...overrides,
...
export class MockBroker {
     ...
     */
    _contractRegistry = new Map();
    _cash;
    _realizedPnL = new Decimal(0);
    _nextOrderId = 1;
    _accountOverride = null;
    _callLog = [];
    _failRemaining = 0;
    _failMethods = new Set();
    constructor(options = {}) {
    ...
    // ---- Lifecycle ----
    async init() { this._record('init', []); this._checkFail('init'); }
    async close() { this._record('close', []); }
    // ---- Contract search (stub) ----
    async searchContracts(_pattern) {
        this._record('searchContracts', [_pattern]);
        const desc = new ContractDescription();
        desc.contract = makeContract();
        return [desc];
    }
    async getContractDetails(_query) {
    ...
    async modifyOrder(orderId, changes) {
        ...
        if (changes.orderType)
            internal.order.orderType = changes.orderType;
        if (changes.tif)
            internal.order.tif = changes.tif;
        const orderState = new OrderState();
        orderState.status = 'Submitted';
        return { success: true, orderId, orderState };
    }
    ...
    async getAccount() {
        ...
        if (this._accountOverride)
            return this._accountOverride;
    ...
    async getOrder(orderId, _symbolHint) {
    ...
    async getOpenOrders() {
        ...
        for (const internal of this._orders.values()) {
            if (internal.status === 'Submitted')
                open.push(this._toOpenOrder(internal));
        }
        return open;
    }
    _toOpenOrder(internal) {
        const orderState = new OrderState();
    ...
    tickPrice(nativeKey, deltaPercent) {
    ...
    fillOrder(orderId, opts = {}) {
        ...
        if (fillQty.lte(0))
            throw new Error('fillOrder: qty must be positive');
    ...
    cancelPendingOrder(orderId) {
        const internal = this._orders.get(orderId);
        if (!internal)
            throw new Error(`MockBroker[${this.id}]: order ${orderId} not found`);
        internal.status = 'Cancelled';
    }
    /**
     ...
     */
    _buildContract(nativeKey, partial) {
        const right = partial?.right;
        const c = buildContract({
            symbol: partial?.symbol || nativeKey,
            secType: partial?.secType || 'CRYPTO',
    ...
    externalDeposit(params) {
        ...
        if (qty.lte(0))
            throw new Error('externalDeposit: quantity must be positive');
    ...
    externalTrade(params) {
        ...
        if (qty.lte(0))
            throw new Error('externalTrade: quantity must be positive');
        const existing = this._positions.get(params.nativeKey);
        if (!existing) {
     ...
     */
    _matchPendingOrders(nativeKey, price) {
        const filled = [];
        for (const internal of this._orders.values()) {
            if (internal.status !== 'Submitted')
                continue;
            if (this.getNativeKey(internal.contract) !== nativeKey)
                continue;
    ...
    setFailMethod(method) {
        this._failMethods.add(method);
    }
    clearFailMethod(method) {
    ...
    // ==================== Internal ====================
    _applyFill(contract, side, qty, price) {
        ...
        const isIncreasing = (existing.side === 'long' && side === 'BUY') ||
