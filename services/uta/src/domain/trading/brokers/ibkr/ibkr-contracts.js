...
export function makeContract(symbol, secType = 'STK', exchange = 'SMART', currency = 'USD') {
 ...
 */
export function classifyIbkrError(code, msg) {
    const classified = classifyCode(code, msg);
    return new BrokerError(classified, `IBKR error ${code}: ${msg}`);
}
function classifyCode(code, msg) {
    // Network / connectivity
    if (code === 502)
        return 'NETWORK'; // Couldn't connect to TWS
    if (code === 504)
        return 'NETWORK'; // Not connected
    if (code === 1100)
        return 'NETWORK'; // Connectivity between IB and TWS has been lost
    if (code === 1101)
        return 'NETWORK'; // Connectivity restored (data maintained)
    if (code === 1102)
        return 'NETWORK'; // Connectivity restored (data lost)
    // Authentication
    if (code === 326)
        return 'AUTH'; // Unable to connect as client ID is already in use
    // Market closed — check message content for TWS order warnings
    if (code === 399 && /outside.*trading.*hours/i.test(msg))
        return 'MARKET_CLOSED';
    if (/market.*closed|not.*open|trading.*halt/i.test(msg))
        return 'MARKET_CLOSED';
    // Exchange-level rejections
    if (code === 200)
        return 'EXCHANGE'; // No security definition found
    if (code === 201)
        return 'EXCHANGE'; // Order rejected
    if (code === 202)
        return 'EXCHANGE'; // Order cancelled
    if (code === 103)
        return 'EXCHANGE'; // Duplicate order id
    if (code === 104)
        return 'EXCHANGE'; // Can't modify a filled order
    if (code === 105)
        return 'EXCHANGE'; // Order being modified doesn't match
    if (code === 110)
        return 'EXCHANGE'; // Price does not conform to min tick
    if (code === 135)
        return 'EXCHANGE'; // Can't find order
    if (code === 136)
        return 'EXCHANGE'; // Can't cancel order (already cancelled/filled)
    if (code === 161)
        return 'EXCHANGE'; // Cancel attempted when not connected
    if (code === 162)
        return 'EXCHANGE'; // Historical data query error
    if (code === 354)
        return 'EXCHANGE'; // Market data not subscribed
    return 'UNKNOWN';
}
//# sourceMappingURL=ibkr-contracts.js.map