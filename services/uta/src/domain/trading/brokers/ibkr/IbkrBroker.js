...
import { EClient, Contract, Order, OrderCancel, OrderState, ContractDescription, } from '@traderalice/ibkr';
...
export class IbkrBroker {
    ...
    _ensureAlive() {
        if (this.bridge?.connectionDead) {
            throw new BrokerError('NETWORK', 'TWS/Gateway connection lost — reconnect pending. Cached data may be stale; orders will NOT transmit.');
        }
    }
    startHeartbeat() {
    ...
    async searchContracts(pattern) {
        ...
        const promise = this.bridge.request(reqId);
    ...
    async expandCashHub(hub, pairCurrency) {
        const q = new Contract();
        q.symbol = hub.symbol;
        q.secType = 'CASH';
        if (pairCurrency)
            q.currency = pairCurrency;
        try {
    ...
    async contractDetailsQuery(query) {
        ...
        const reqId = this.bridge.allocReqId();
        const promise = this.bridge.requestCollector(reqId);
     ...
     */
    async expandContract(nativeKey, filters = {}) {
        ...
        if (nativeKey.startsWith('issuer:')) {
            ...
            const all = details.map((d) => {
                ...
                if (d.maturity && !c.lastTradeDateOrContractMonth)
                    c.lastTradeDateOrContractMonth = d.maturity;
                return c;
            });
        ...
        if (isNaN(asNum) || String(asNum) !== nativeKey) {
            throw new BrokerError('EXCHANGE', `Cannot expand "${nativeKey}" — expansion takes a conId aliceId (an underlying from search) or an issuer: directory key.`);
        }
        ...
        if (famSecType === 'FUT' || filters.expiry) {
            ...
            if (famSecType === 'OPT')
                q.exchange = 'SMART';
            if (filters.expiry)
                q.lastTradeDateOrContractMonth = filters.expiry;
            if (filters.right)
                q.right = filters.right;
            const details = await this.contractDetailsQuery(q);
            let all = details.map((d) => d.contract);
            if (filters.strikeMin != null)
                all = all.filter((c) => c.strike >= filters.strikeMin);
            if (filters.strikeMax != null)
                all = all.filter((c) => c.strike <= filters.strikeMax);
            all.sort((a, b) => (a.lastTradeDateOrContractMonth || '').localeCompare(b.lastTradeDateOrContractMonth || '')
                || (a.strike - b.strike)
    ...
    async modifyOrder(orderId, changes) {
        try {
            ...
            if (changes.totalQuantity != null)
                mergedOrder.totalQuantity = changes.totalQuantity;
            if (changes.lmtPrice != null)
                mergedOrder.lmtPrice = changes.lmtPrice;
            if (changes.auxPrice != null)
                mergedOrder.auxPrice = changes.auxPrice;
            if (changes.tif)
                mergedOrder.tif = changes.tif;
            if (changes.orderType)
                mergedOrder.orderType = changes.orderType;
            if (changes.trailingPercent != null)
                mergedOrder.trailingPercent = changes.trailingPercent;
            if (changes.trailStopPrice != null)
                mergedOrder.trailStopPrice = changes.trailStopPrice;
    ...
    async cancelOrder(orderId, orderCancel) {
        try {
            this._ensureAlive();
            const numericId = parseInt(orderId, 10);
            const promise = this.bridge.requestOrder(numericId);
    ...
    async closePosition(contract, quantity) {
        ...
        const pos = positions.find(p => (contract.conId && p.contract.conId === contract.conId) ||
    ...
    async getAccount() {
        ...
        if (!download)
            throw new BrokerError('NETWORK', 'Account data not yet available');
    ...
    getNativeKey(contract) {
        ...
        if (contract.conId)
            return String(contract.conId);
        if (contract.secType === 'BOND' && contract.issuerId)
            return `issuer:${contract.issuerId}`;
        return contract.symbol;
    }
    resolveNativeKey(nativeKey) {
        if (nativeKey.startsWith('issuer:')) {
            throw new Error(`"${nativeKey}" is a bond-issuer directory, not a tradeable contract — ` +
...
//# sourceMappingURL=IbkrBroker.js.map