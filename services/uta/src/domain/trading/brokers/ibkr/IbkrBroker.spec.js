 ...
 */
function bareBroker() {
    return Object.create(IbkrBroker.prototype);
}
function stkOrder() {
...
describe('IbkrBroker — attached TP/SL refusal gate', () => {
    ...
    it('an empty tpsl object does not trip the gate', async () => {
        ...
        await expect(async () => {
            ...
            if (r.success === false && /refusing/i.test(r.error ?? ''))
                throw new Error('gate tripped');
            return r;
...
describe('IbkrBroker — getAccount mixed-currency math (ANG-101 / issues #295 #314)', () => {
    function brokerWithCache(values, positions) {
        const b = bareBroker();
        b.bridge = {
...
//# sourceMappingURL=IbkrBroker.spec.js.map