...
function score(query, entry) {
    ...
    if (sym === q || base === q || name === q)
        return 100;
    if ((sym && sym.startsWith(q)) || (base && base.startsWith(q)))
        return 80;
    ...
    if (name && wbRe.test(name))
        return 50;
    if (sym.includes(q) || local.includes(q) || name.includes(q))
        return 30;
...
export function fuzzyRankContracts(entries, query, options = {}) {
    const q = query.trim();
    if (!q)
        return [];
...
//# sourceMappingURL=fuzzy-rank.js.map