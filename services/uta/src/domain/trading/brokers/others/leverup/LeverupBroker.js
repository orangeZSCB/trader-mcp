...
import { findPairBySymbol, getPairs } from './pairs.js';
import { qtyToWei, weiToQty, priceToWei, weiToPrice, amountInToWei, weiToAmountIn, USDC_DECIMALS, } from './decimals.js';
import { accountFromPrivateKey, generateOpenSalt, signOpenPosition, signClosePosition, } from './eip712.js';
...
export class LeverupBroker {
    ...
    async searchContracts(pattern) {
        ...
        const matches = getPairs(this.network).filter(p => p.base.toUpperCase().includes(upper) ||
    ...
    async closePosition(contract, _quantity) {
        this.ensureInit();
        const pair = this.resolvePair(contract);
        if (!pair) {
            return { success: false, error: `Unknown LeverUp pair: ${contract.localSymbol ?? contract.symbol}` };
    ...
    resolveNativeKey(nativeKey) {
        ...
        if (pair)
            return this.pairToContract(pair);
    ...
    async _fetchOpenPositionsRaw() {
        this.ensureInit();
        return this.reader.fetchOpenPositions(this.traderAddress);
    }
}
//# sourceMappingURL=LeverupBroker.js.map