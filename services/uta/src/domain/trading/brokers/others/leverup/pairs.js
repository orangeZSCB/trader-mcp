 ...
 */
export const TESTNET_PAIRS = MAINNET_PAIRS;
export function getPairs(network) {
    return network === 'testnet' ? TESTNET_PAIRS : MAINNET_PAIRS;
}
export function findPairBySymbol(network, symbol) {
    return getPairs(network).find(p => p.symbol === symbol);
}
export function findPairByPairBase(network, pairBase) {
