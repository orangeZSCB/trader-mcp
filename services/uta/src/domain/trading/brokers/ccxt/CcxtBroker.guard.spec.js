...
describe('CcxtBroker construct-time demo/sandbox guards', () => {
    it('throws a clear CONFIG error for an exchange whose demo has no endpoint (okx)', () => {
        ...
        expect(() => new CcxtBroker({ id: 't', exchange: 'okx', sandbox: false, demoTrading: true, apiKey: 'k', secret: 's', password: 'p' })).toThrow(/no CCXT demo-trading endpoint/);
    });
    it('constructs cleanly for an exchange that supports demo trading (binance)', () => {
        ...
        expect(() => new CcxtBroker({ id: 't', exchange: 'binance', sandbox: false, demoTrading: true, apiKey: 'k', secret: 's' })).not.toThrow();
    });
    it('throws CONFIG when demo + sandbox are combined (CCXT NotSupported)', () => {
        ...
        expect(() => new CcxtBroker({ id: 't', exchange: 'binance', sandbox: true, demoTrading: true, apiKey: 'k', secret: 's' })).toThrow(/cannot enable Demo Trading/);
    });
    it('throws a clear CONFIG error for sandbox on an exchange with no testnet URL', () => {
        ...
        expect(() => new CcxtBroker({ id: 't', exchange: 'kucoin', sandbox: true, demoTrading: false, apiKey: 'k', secret: 's', password: 'p' })).toThrow(/cannot enable Sandbox/);
    });
});
//# sourceMappingURL=CcxtBroker.guard.spec.js.map