...
export const CCXT_TIMEFRAME = {
    '1m': '1m', '5m': '5m', '15m': '15m', '30m': '30m',
    '1h': '1h', '4h': '4h', '1d': '1d', '1w': '1w',
};
// ---- Type mapping ----
export function ccxtTypeToSecType(type) {
    switch (type) {
...
export function makeOrderState(ccxtStatus) {
 ...
 */
export function marketToContract(market, exchangeName) {
    const secType = ccxtTypeToSecType(market.type);
    ...
    if (secType === 'OPT' || secType === 'FOP') {
        if (m.strike != null)
            params.strike = m.strike;
        if (m.optionType === 'call')
            params.right = 'C';
        else if (m.optionType === 'put')
            params.right = 'P';
    }
 ...
 */
export function contractToCcxt(contract, markets, exchangeName) {
...
export function resolveContractSync(query, markets) {
    if (!query.symbol)
        return [];
    ...
    for (const market of Object.values(markets)) {
        ...
        if (!market.base || !market.quote)
            continue;
        if (market.base.toUpperCase() !== searchBase)
            continue;
        if (query.secType) {
            const marketSecType = ccxtTypeToSecType(market.type);
            if (marketSecType !== query.secType)
                continue;
        }
        if (query.currency && market.quote.toUpperCase() !== query.currency.toUpperCase())
            continue;
        if (!query.currency) {
...
//# sourceMappingURL=ccxt-contracts.js.map