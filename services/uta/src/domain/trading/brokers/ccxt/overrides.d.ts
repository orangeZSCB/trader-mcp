...
export interface CcxtExchangeOverrides {
    ...
    fetchOrderById?(exchange: Exchange, orderId: string, symbol: string, defaultImpl: DefaultImpl<[Exchange, string, string], CcxtOrder>): Promise<CcxtOrder>;
    /** Cancel an order by ID (regular + conditional). */
    cancelOrderById?(exchange: Exchange, orderId: string, symbol: string | undefined, defaultImpl: DefaultImpl<[Exchange, string, string | undefined], void>): Promise<void>;
    ...
    placeOrder?(exchange: Exchange, symbol: string, type: string, side: 'buy' | 'sell', amount: number, price: number | undefined, params: Record<string, unknown>, defaultImpl: DefaultImpl<[
        Exchange,
        string,
        string,
        'buy' | 'sell',
    ...
    ], CcxtOrder>): Promise<CcxtOrder>;
    /** Fetch positions. Override when CCXT's parsePosition leaves important
     *  fields undefined (e.g. hyperliquid omits markPrice). */
    fetchPositions?(exchange: Exchange, defaultImpl: DefaultImpl<[Exchange], CcxtPosition[]>): Promise<CcxtPosition[]>;
    ...
    placeOrderWithTpSl?(exchange: Exchange, symbol: string, type: string, side: 'buy' | 'sell', amount: number, price: number | undefined, tpsl: {
        takeProfit?: {
            price: string;
        };
        stopLoss?: {
    ...
    }, params: Record<string, unknown>): Promise<CcxtOrder>;
    ...
    fetchAllOpenOrders?(exchange: Exchange, defaultImpl: DefaultImpl<[Exchange], CcxtOrder[]>): Promise<CcxtOrder[]>;
}
/** Default: fetchOrder + { stop: true } fallback. Works for binance, okx, bitget, etc. */
export declare function defaultFetchOrderById(exchange: Exchange, orderId: string, symbol: string): Promise<CcxtOrder>;
/** Default: cancelOrder + { stop: true } fallback. */
export declare function defaultCancelOrderById(exchange: Exchange, orderId: string, symbol?: string): Promise<void>;
/** Default: pass straight through to ccxt.createOrder. Works for bybit, binance, alpaca-via-ccxt, etc. */
export declare function defaultPlaceOrder(exchange: Exchange, symbol: string, type: string, side: 'buy' | 'sell', amount: number, price: number | undefined, params: Record<string, unknown>): Promise<CcxtOrder>;
/** Default: pass straight through to ccxt.fetchPositions. */
export declare function defaultFetchPositions(exchange: Exchange): Promise<CcxtPosition[]>;
/**
 ...
 */
export declare function defaultFetchAllOpenOrders(exchange: Exchange): Promise<CcxtOrder[]>;
export declare const exchangeOverrides: Record<string, CcxtExchangeOverrides>;
export {};
