...
export function createCcxtProviderTools(manager) {
    /** Resolve to exactly one CCXT UTA. Returns error object if unable. */
    const resolveCcxtOne = (source) => {
        const targets = manager.resolve(source)
            .filter((uta) => uta.broker instanceof CcxtBroker);
        if (targets.length === 0)
            return { error: 'No CCXT account available.' };
        if (targets.length > 1) {
    ...
    const sourceDesc = 'Account source — matches account id or provider name. Auto-resolves if only one CCXT account exists.';
    return {
        getFundingRate: tool({
...
Use searchContracts first to get the aliceId.`,
            ...
            execute: async ({ aliceId, source }) => {
                ...
                let contract;
                try {
                    contract = uta.contractFromAliceId(aliceId);
                }
                catch (err) {
...
//# sourceMappingURL=ccxt-tools.js.map