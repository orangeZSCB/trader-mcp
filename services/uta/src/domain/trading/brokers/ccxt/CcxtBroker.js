...
import { CCXT_CREDENTIAL_FIELDS } from './ccxt-types.js';
import { MAX_INIT_RETRIES, INIT_RETRY_BASE_MS } from './ccxt-types.js';
import { makeOrderState, marketToContract, contractToCcxt, CCXT_TIMEFRAME, } from './ccxt-contracts.js';
import { fuzzyRankContracts } from '../fuzzy-rank.js';
import { exchangeOverrides, defaultFetchOrderById, defaultCancelOrderById, defaultPlaceOrder, defaultFetchPositions, defaultFetchAllOpenOrders, } from './overrides.js';
...
export class CcxtBroker {
    ...
    orderSymbolCache = new Map();
    warnedOpenOrdersUnsupported = false;
    constructor(config) {
        ...
        if (!this.keyless)
            try {
                ...
                const required = Object.entries(this.exchange.requiredCredentials ?? {})
                    ...
                    .map(([k]) => k);
                const missing = required.filter(k => !this.exchange[k]);
                throw new BrokerError('CONFIG', `${this.exchangeName} requires credentials: ${required.join(', ')}. Missing: ${missing.join(', ') || 'unknown'}. (${err instanceof Error ? err.message : String(err)})`);
            }
        const origFetchMarkets = this.exchange.fetchMarkets.bind(this.exchange);
        const accountId = this.id;
        this.exchange.fetchMarkets = async (params) => {
            const ex = this.exchange;
            const opts = (ex['options'] ?? {});
            const fmOpts = (opts['fetchMarkets'] ?? {});
            ...
            try {
                ...
                for (const type of types) {
                    ...
                    if (!success) {
                        ...
                        throw new Error(`CcxtBroker[${accountId}]: fetchMarkets(${type}) failed after ${MAX_INIT_RETRIES} attempts: ${msg}`);
                    }
                }
                return allMarkets;
            }
    ...
    async searchContracts(pattern) {
        ...
        for (const market of Object.values(this.markets)) {
            ...
            if (quote !== 'USDT' && quote !== 'USD' && quote !== 'USDC')
                continue;
        ...
        const typeOrder = { swap: 0, future: 1, spot: 2, option: 3 };
        const quoteOrder = { USDT: 0, USD: 1, USDC: 2 };
        candidates.sort((a, b) => {
        ...
        // without being noisy.
        const ranked = fuzzyRankContracts(candidates.flatMap((m) => {
            try {
        ...
        const derivativeTypes = new Set();
        for (const desc of ranked) {
            const m = desc.contract.localSymbol ? this.markets[desc.contract.localSymbol] : undefined;
            if (!m)
                continue;
            if (m.type === 'future')
                derivativeTypes.add('FUT');
            if (m.type === 'option')
                derivativeTypes.add('OPT');
        }
        const derivativeSecTypes = derivativeTypes.size > 0 ? Array.from(derivativeTypes) : [];
        for (const desc of ranked)
    ...
    // ---- Trading operations ----
    async placeOrder(contract, order, tpsl, extraParams) {
        this.ensureInit();
        ...
        if (tpsl?.takeProfit || tpsl?.stopLoss) {
            ...
            if (!attachOverride) {
                return {
                    success: false,
                    error: `Attached TP/SL is not verified to reach ${this.exchangeName} through ccxt — refusing rather than ` +
        ...
        try {
            ...
            if (order.orderType === 'STP' || order.orderType === 'STP LMT') {
                ...
                return {
                    success: false,
                    error: `${order.orderType} is not verified to reach ${this.exchangeName} through ccxt — refusing rather ` +
                        `than risking a silently mis-typed order. Use STP / STP LMT, or manage the trail manually.`,
                };
            }
            const side = order.action.toLowerCase();
    ...
    async modifyOrder(orderId, changes) {
        ...
        try {
            ...
            if (changes.auxPrice != null && !changes.auxPrice.equals(UNSET_DECIMAL))
                params.stopPrice = changes.auxPrice.toNumber();
            if (changes.trailStopPrice != null && !changes.trailStopPrice.equals(UNSET_DECIMAL))
                params.trailStopPrice = changes.trailStopPrice.toNumber();
            if (changes.trailingPercent != null && !changes.trailingPercent.equals(UNSET_DECIMAL))
                params.trailingPercent = changes.trailingPercent.toNumber();
            if (changes.tif)
                params.timeInForce = changes.tif.toLowerCase();
            const result = await this.exchange.editOrder(orderId, ccxtSymbol, changes.orderType ? ibkrOrderTypeToCcxt(changes.orderType) : (original.type ?? 'market'), original.side, qty, price, params);
            return {
                success: true,
                orderId: result.id,
    ...
    async closePosition(contract, quantity) {
        ...
        const pos = positions.find(p => {
            ...
            if (ccxtSymbol && posWire === ccxtSymbol)
                return true;
     ...
     */
    async fetchSpotHoldings(prefetched) {
        const balance = prefetched ?? await this.exchange.fetchBalance();
        const bal = balance;
        const holdings = [];
        for (const [coin, entry] of Object.entries(bal)) {
            if (BALANCE_RESERVED_KEYS.has(coin))
                continue;
            if (STABLECOIN_TO_USD.has(coin.toUpperCase()))
                continue;
        ...
        try {
            ...
            for (const s of symbols) {
                try {
                    tickers[s] = await this.exchange.fetchTicker(s);
                }
                catch {
    ...
    async getAccount() {
        ...
        try {
            ...
            for (const p of rawPositions) {
                ...
                realizedPnL = realizedPnL.plus(new Decimal(String(p.realizedPnl ?? 0)));
        ...
        try {
            ...
            for (const p of raw) {
                ...
                result.push(buildPosition({
                    ...
                    marketValue: marketValue.toString(),
                    unrealizedPnL: unrealizedPnL.toString(),
                    realizedPnL: new Decimal(String(p.realizedPnl ?? 0)).toString(),
    ...
    async getOrder(orderId, symbolHint) {
        this.ensureInit();
    ...
    convertCcxtOrder(o) {
        ...
        if (o.price != null)
            order.lmtPrice = new Decimal(o.price);
    ...
    async getOpenOrders() {
        ...
        try {
            ...
            if (!this.warnedOpenOrdersUnsupported) {
                ...
                console.warn(`CcxtBroker[${this.id}]: fetchOpenOrders unavailable — external-order observation disabled for this account ` +
    ...
    async getQuote(contract) {
        ...
        const ccxtSymbol = contractToCcxt(contract, this.markets, this.exchangeName);
        if (!ccxtSymbol)
            throw new BrokerError('EXCHANGE', 'Cannot resolve contract to CCXT symbol');
        try {
            const ticker = await this.exchange.fetchTicker(ccxtSymbol);
            const market = this.markets[ccxtSymbol];
            return {
    ...
    async getHistorical(contract, params) {
        ...
        const supported = this.exchange.timeframes;
    ...
    resolveNativeKey(nativeKey) {
        ...
        if (market)
            return marketToContract(market, this.exchange.id);
        ...
        try {
            ...
            return {
                contract: market
                    ? marketToContract(market, this.exchangeName)
                    : contract,
                bids: book.bids.map(([p, a]) => [p ?? 0, a ?? 0]),
                asks: book.asks.map(([p, a]) => [p ?? 0, a ?? 0]),
...
//# sourceMappingURL=CcxtBroker.js.map