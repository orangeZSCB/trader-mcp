...
// ==================== Helpers ====================
function makeSpotMarket(base, quote, symbol) {
    return {
...
function makeSwapMarket(base, quote, symbol) {
    return {
        id: symbol ?? `${base}${quote}`,
...
function setInitialized(acc, markets) {
    ;
    acc.initialized = true;
    acc.exchange.markets = markets;
}
// ==================== Constructor ====================
describe('CcxtBroker — constructor', () => {
    it('throws for unknown exchange', () => {
        expect(() => new CcxtBroker({ exchange: 'unknownxyz', apiKey: 'k', secret: 's', sandbox: false })).toThrow('Unknown CCXT exchange');
    });
...
describe('CcxtBroker — searchContracts', () => {
    ...
    it('only returns USDT/USD/USDC quoted markets', async () => {
        ;
        acc.exchange.markets['BTC/DOGE'] = { ...makeSpotMarket('BTC', 'DOGE'), id: 'BTCDOGE' };
        const results = await acc.searchContracts('BTC');
    ...
    it('excludes inactive markets', async () => {
        ;
        acc.exchange.markets['BTC/USDC'] = { ...makeSpotMarket('BTC', 'USDC'), active: false };
    ...
    it('sorts swap before spot by default', async () => {
        ...
        expect(first.contract.secType ?? first.contract.symbol.includes(':') ? 'CRYPTO_PERP' : 'CRYPTO').toBeTruthy();
    });
});
...
describe('CcxtBroker — cancelOrder cache', () => {
    it('calls exchange.cancelOrder with undefined symbol when orderId is not in cache', async () => {
        const acc = makeAccount();
        setInitialized(acc, {});
        acc.exchange.cancelOrder = vi.fn().mockResolvedValue({});
        await acc.cancelOrder('order-not-cached');
        expect(acc.exchange.cancelOrder).toHaveBeenCalledWith('order-not-cached', undefined);
    });
    it('returns PlaceOrderResult with error when exchange.cancelOrder throws', async () => {
        const acc = makeAccount();
        setInitialized(acc, {});
        acc.exchange.cancelOrder = vi.fn().mockRejectedValue(new Error('symbol required'));
    ...
    it('returns PlaceOrderResult with Cancelled status when orderId is cached', async () => {
        const acc = makeAccount();
        setInitialized(acc, {});
        acc.orderSymbolCache.set('order-123', 'BTC/USDT:USDT');
        acc.exchange.cancelOrder = vi.fn().mockResolvedValue({});
        const result = await acc.cancelOrder('order-123');
        expect(result.success).toBe(true);
        expect(result.orderId).toBe('order-123');
        expect(result.orderState?.status).toBe('Cancelled');
        expect(acc.exchange.cancelOrder).toHaveBeenCalledWith('order-123', 'BTC/USDT:USDT');
    });
});
...
describe('CcxtBroker — placeOrder notional', () => {
    it('converts notional to size using ticker price when qty is not provided', async () => {
        ...
        acc.exchange.fetchTicker = vi.fn().mockResolvedValue({ last: 50_000 });
        acc.exchange.createOrder = vi.fn().mockResolvedValue({
        ...
        expect(result.success).toBe(true);
        const createOrderCall = acc.exchange.createOrder.mock.calls[0];
...
describe('CcxtBroker — getOrder (bybit)', () => {
    it('uses fetchOpenOrder for open orders', async () => {
        ...
        acc.orderSymbolCache.set('ord-100', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
        ...
        expect(acc.exchange.fetchOpenOrder).toHaveBeenCalledWith('ord-100', 'ETH/USDT:USDT');
        // Should NOT use fetchOrder (bybit override avoids it)
        expect(acc.exchange.fetchOrder).not.toHaveBeenCalled();
    });
    it('falls back to fetchClosedOrder for filled orders', async () => {
        ...
        setInitialized(acc, { 'ETH/USDT:USDT': market });
        acc.orderSymbolCache.set('ord-100', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockRejectedValue(new Error('not open'));
        acc.exchange.fetchClosedOrder = vi.fn().mockResolvedValue({
    ...
    it('finds conditional orders via { stop: true } fallback', async () => {
        ...
        const market = makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT');
        setInitialized(acc, { 'ETH/USDT:USDT': market });
        acc.orderSymbolCache.set('ord-sl', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn()
        ...
        acc.exchange.fetchClosedOrder = vi.fn().mockRejectedValue(new Error('not found'));
        const result = await acc.getOrder('ord-sl');
        expect(result).not.toBeNull();
        expect(result.orderState.status).toBe('Submitted');
        // Second fetchOpenOrder call should have { stop: true }
        expect(acc.exchange.fetchOpenOrder).toHaveBeenCalledWith('ord-sl', 'ETH/USDT:USDT', { stop: true });
    });
    ...
    it('returns null when order not found on any endpoint', async () => {
        ...
        acc.orderSymbolCache.set('ord-404', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockRejectedValue(new Error('not found'));
        acc.exchange.fetchClosedOrder = vi.fn().mockRejectedValue(new Error('not found'));
    ...
    it('extracts tpsl from CCXT order with takeProfitPrice/stopLossPrice', async () => {
        ...
        const market = makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT');
        setInitialized(acc, { 'ETH/USDT:USDT': market });
        acc.orderSymbolCache.set('ord-tp', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
    ...
    it('returns no tpsl when CCXT order has no TP/SL prices', async () => {
        ...
        const market = makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT');
        setInitialized(acc, { 'ETH/USDT:USDT': market });
        acc.orderSymbolCache.set('ord-plain', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
    ...
    it('extracts only takeProfit when stopLossPrice is absent', async () => {
        ...
        const market = makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT');
        setInitialized(acc, { 'ETH/USDT:USDT': market });
        acc.orderSymbolCache.set('ord-tp-only', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
...
describe('CcxtBroker — getOrder (default/binance)', () => {
    it('uses fetchOrder for regular orders', async () => {
        ...
        setInitialized(acc, { 'ETH/USDT:USDT': market });
        acc.orderSymbolCache.set('ord-100', 'ETH/USDT:USDT');
        acc.exchange.fetchOrder = vi.fn().mockResolvedValue({
            id: 'ord-100', symbol: 'ETH/USDT:USDT', side: 'sell', amount: 0.5,
            type: 'market', price: null, status: 'closed',
        });
        const result = await acc.getOrder('ord-100');
        ...
        expect(result.order.action).toBe('SELL');
        expect(result.orderState.status).toBe('Filled');
        expect(acc.exchange.fetchOrder).toHaveBeenCalledWith('ord-100', 'ETH/USDT:USDT');
    });
    it('falls back to { stop: true } for conditional orders', async () => {
        ...
        expect(result).not.toBeNull();
        expect(result.orderState.status).toBe('Submitted');
        expect(acc.exchange.fetchOrder).toHaveBeenCalledTimes(2);
        expect(acc.exchange.fetchOrder).toHaveBeenLastCalledWith('ord-sl', 'ETH/USDT:USDT', { stop: true });
    });
    it('returns null when order not found on either endpoint', async () => {
        const acc = makeAccount({ exchange: 'binance' });
        setInitialized(acc, { 'ETH/USDT:USDT': makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT') });
        acc.orderSymbolCache.set('ord-404', 'ETH/USDT:USDT');
        acc.exchange.fetchOrder = vi.fn().mockRejectedValue(new Error('not found'));
        const result = await acc.getOrder('ord-404');
        expect(result).toBeNull();
        expect(acc.exchange.fetchOrder).toHaveBeenCalledTimes(2);
    });
...
describe('CcxtBroker — placeOrder qty-based', () => {
    ...
    it('places market order with totalQuantity', async () => {
        ...
        const call = acc.exchange.createOrder.mock.calls[0];
...
describe('CcxtBroker — modifyOrder', () => {
    it('calls exchange.editOrder with mapped fields', async () => {
        ...
        acc.orderSymbolCache.set('ord-100', 'BTC/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
            type: 'limit', side: 'buy', amount: 0.5, price: 60000,
        });
        acc.exchange.editOrder = vi.fn().mockResolvedValue({
        ...
        const call = acc.exchange.editOrder.mock.calls[0];
...
describe('CcxtBroker — modifyOrder field forwarding', () => {
    it('uses original price when lmtPrice not in changes (Partial<Order>)', async () => {
        const acc = makeAccount();
        setInitialized(acc, { 'ETH/USDT:USDT': makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT') });
        acc.orderSymbolCache.set('ord-200', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
            type: 'limit', side: 'buy', amount: 0.1, price: 1900,
        });
        acc.exchange.editOrder = vi.fn().mockResolvedValue({ id: 'ord-200-edited', status: 'open' });
        // Partial<Order> — only totalQuantity, no lmtPrice
        const changes = { totalQuantity: new Decimal(0.2) };
    ...
    it('forwards auxPrice as stopPrice in params', async () => {
        const acc = makeAccount();
        setInitialized(acc, { 'ETH/USDT:USDT': makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT') });
        acc.orderSymbolCache.set('ord-300', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
            type: 'limit', side: 'sell', amount: 0.1, price: 2100,
        });
        acc.exchange.editOrder = vi.fn().mockResolvedValue({ id: 'ord-300-edited', status: 'open' });
        const changes = { auxPrice: new Decimal(1850) };
    ...
    it('forwards tif in params', async () => {
        const acc = makeAccount();
        setInitialized(acc, { 'ETH/USDT:USDT': makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT') });
        acc.orderSymbolCache.set('ord-400', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
            type: 'limit', side: 'buy', amount: 0.1, price: 1900,
        });
        acc.exchange.editOrder = vi.fn().mockResolvedValue({ id: 'ord-400-edited', status: 'open' });
        const changes = { tif: 'GTC' };
    ...
    it('does not include undefined fields in params', async () => {
        const acc = makeAccount();
        setInitialized(acc, { 'ETH/USDT:USDT': makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT') });
        acc.orderSymbolCache.set('ord-500', 'ETH/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn().mockResolvedValue({
            type: 'limit', side: 'buy', amount: 0.1, price: 1900,
        });
        acc.exchange.editOrder = vi.fn().mockResolvedValue({ id: 'ord-500-edited', status: 'open' });
        // Only change qty — nothing else should appear in params
        const changes = { totalQuantity: new Decimal(0.5) };
...
describe('CcxtBroker — closePosition', () => {
    it('reverses position with market order and correct side', async () => {
        ...
        const market = makeSwapMarket('BTC', 'USDT', 'BTC/USDT:USDT');
        setInitialized(acc, { 'BTC/USDT:USDT': market });
        acc.exchange.fetchPositions = vi.fn().mockResolvedValue([
            {
...
describe('CcxtBroker — precision', () => {
    it('placeOrder sends precise quantity (no float corruption)', async () => {
        const acc = makeAccount();
        setInitialized(acc, { 'ETH/USDT:USDT': makeSwapMarket('ETH', 'USDT', 'ETH/USDT:USDT') });
        acc.exchange.createOrder = vi.fn().mockResolvedValue({ id: 'ord-1', status: 'open' });
        const contract = new Contract();
        contract.localSymbol = 'ETH/USDT:USDT';
        const order = new Order();
        order.action = 'BUY';
        ...
        const amount = acc.exchange.createOrder.mock.calls[0][3];
...
describe('CcxtBroker — closePosition reduceOnly', () => {
    it('passes reduceOnly: true to createOrder params', async () => {
        ...
        acc.exchange.createOrder = vi.fn().mockResolvedValue({ id: 'close-1', status: 'closed' });
        const contract = new Contract();
        contract.localSymbol = 'ETH/USDT:USDT';
        await acc.closePosition(contract);
        // createOrder 6th arg is params
        const params = acc.exchange.createOrder.mock.calls[0][5];
...
describe('CcxtBroker — getAccount', () => {
    it('maps CCXT balance to AccountInfo', async () => {
        const acc = makeAccount();
        setInitialized(acc, {});
        acc.exchange.fetchBalance = vi.fn().mockResolvedValue({
    ...
    it('includes spot holdings value in netLiquidation', async () => {
        ...
        acc.exchange.fetchPositions = vi.fn().mockResolvedValue([]);
        acc.exchange.fetchTickers = vi.fn().mockResolvedValue({
            'BTC/USDT': { last: 60000 },
        });
        const info = await acc.getAccount();
...
describe('CcxtBroker — getPositions', () => {
    ...
    it('falls back to per-symbol fetchTicker when fetchTickers throws', async () => {
        ...
        acc.exchange.fetchTickers = vi.fn().mockRejectedValue(new Error('not supported'));
        acc.exchange.fetchTicker = vi.fn().mockResolvedValue({ last: 60000 });
        const positions = await acc.getPositions();
        expect(positions).toHaveLength(1);
        expect(positions[0].marketPrice).toBe('60000');
        expect(acc.exchange.fetchTicker).toHaveBeenCalledWith('BTC/USDT');
    });
});
// ==================== getOrders ====================
describe('CcxtBroker — getOrders', () => {
    it('queries each orderId via getOrder and returns results (bybit)', async () => {
        ...
        const market = makeSwapMarket('BTC', 'USDT', 'BTC/USDT:USDT');
        setInitialized(acc, { 'BTC/USDT:USDT': market });
        acc.orderSymbolCache.set('ord-1', 'BTC/USDT:USDT');
        acc.orderSymbolCache.set('ord-2', 'BTC/USDT:USDT');
        acc.exchange.fetchOpenOrder = vi.fn()
        ...
        acc.exchange.fetchClosedOrder = vi.fn()
...
describe('CcxtBroker — getQuote', () => {
    it('returns mapped ticker data', async () => {
        ...
        setInitialized(acc, { 'BTC/USDT:USDT': market });
        const now = Date.now();
        acc.exchange.fetchTicker = vi.fn().mockResolvedValue({
...
describe('CcxtBroker — getHistorical', () => {
    it('maps ccxt OHLCV rows to string-typed bars', async () => {
        const acc = makeAccount();
        setInitialized(acc, { 'BTC/USDT:USDT': makeSwapMarket('BTC', 'USDT', 'BTC/USDT:USDT') });
        acc.exchange.fetchOHLCV = vi.fn().mockResolvedValue([
        ...
        expect(acc.exchange.fetchOHLCV).toHaveBeenCalledWith('BTC/USDT:USDT', '1d', undefined, 2);
    });
    it('loud-refuses an interval the exchange does not support', async () => {
        const acc = makeAccount();
        setInitialized(acc, { 'BTC/USDT:USDT': makeSwapMarket('BTC', 'USDT', 'BTC/USDT:USDT') });
        acc.exchange.timeframes = { '1d': '1d' }; // no 5m
        const contract = new Contract();
        contract.localSymbol = 'BTC/USDT:USDT';
...
//# sourceMappingURL=CcxtBroker.spec.js.map