...
import { Order, UNSET_DECIMAL, UNSET_INTEGER, UNSET_DOUBLE } from '@traderalice/ibkr';
import { BrokerError } from './brokers/types.js';
const REACH_RANK = { down: 0, connected: 1, readable: 2 };
...
export class UnifiedTradingAccount {
    ...
    static OFFLINE_THRESHOLD = 6;
    static RECOVERY_BASE_MS = 5_000;
    static RECOVERY_MAX_MS = 60_000;
    ...
    constructor(broker, options = {}) {
        ...
        this._getState = async () => {
            ...
            const [accountInfo, positions, orders] = await this._callBroker(() => Promise.all([
                broker.getAccount(),
                broker.getPositions(),
    ...
    get health() {
        ...
        if (this._currentReach === 'down')
            return 'offline';
        if (this._consecutiveFailures >= UnifiedTradingAccount.OFFLINE_THRESHOLD)
            return 'offline';
        ...
        if (!this._reachedTarget())
            return 'degraded';
        if (this._consecutiveFailures >= UnifiedTradingAccount.DEGRADED_THRESHOLD)
            return 'degraded';
        return 'healthy';
    }
    get disabled() {
    ...
    _scheduleRecoveryAttempt(attempt) {
        const delay = Math.min(UnifiedTradingAccount.RECOVERY_BASE_MS * 2 ** attempt, UnifiedTradingAccount.RECOVERY_MAX_MS);
    ...
    _assertWritable() {
        if (this.readOnly) {
            throw new BrokerError('CONFIG', `Account "${this.label}" is read-only${this.keyless ? ' (keyless public-data account)' : ''} — trading operations are not allowed.`);
        }
    }
    /**
    ...
    _validatePlaceOrderParams(p) {
        ...
        if (qty && cash)
            fail('totalQuantity and cashQty are mutually exclusive — provide exactly one.');
        if (p.orderType === 'MKT') {
            if (!qty && !cash)
                fail('requires totalQuantity (shares) or cashQty (notional).');
        }
        else {
            if (cash)
                fail('cashQty (notional) is only supported for MKT orders — use totalQuantity.');
            if (!qty)
                fail('requires totalQuantity.');
        }
        switch (p.orderType) {
            case 'LMT':
                if (!has(p.lmtPrice))
                    fail('requires lmtPrice.');
                break;
            case 'STP':
                if (!has(p.auxPrice))
                    fail('requires auxPrice (stop trigger price).');
                break;
            case 'STP LMT':
                if (!has(p.auxPrice))
                    fail('requires auxPrice (stop trigger price).');
            ...
            case 'TRAIL LIMIT': {
                ...
                if (aux && pct)
                    fail('auxPrice and trailingPercent are mutually exclusive — provide exactly one.');
                if (!aux && !pct)
                    fail('requires auxPrice (trailing offset) or trailingPercent.');
                if (p.orderType === 'TRAIL LIMIT' && !has(p.lmtPrice))
                    fail('requires lmtPrice.');
                break;
            }
        }
    ...
    stagePlaceOrder(params) {
        ...
        if (params.totalQuantity != null)
            order.totalQuantity = new Decimal(String(params.totalQuantity));
        if (params.cashQty != null)
            order.cashQty = new Decimal(String(params.cashQty));
        if (params.lmtPrice != null)
            order.lmtPrice = new Decimal(String(params.lmtPrice));
        if (params.auxPrice != null)
            order.auxPrice = new Decimal(String(params.auxPrice));
        if (params.trailStopPrice != null)
            order.trailStopPrice = new Decimal(String(params.trailStopPrice));
        if (params.trailingPercent != null)
            order.trailingPercent = new Decimal(String(params.trailingPercent));
        if (params.goodTillDate != null)
            order.goodTillDate = params.goodTillDate;
        if (params.outsideRth)
            order.outsideRth = true;
        if (params.parentId != null)
            order.parentId = parseInt(params.parentId, 10) || 0;
        if (params.ocaGroup != null)
            order.ocaGroup = params.ocaGroup;
        const tpsl = (params.takeProfit || params.stopLoss)
    ...
    stageModifyOrder(params) {
        ...
        const changes = {};
        if (params.totalQuantity != null)
            changes.totalQuantity = new Decimal(String(params.totalQuantity));
        if (params.lmtPrice != null)
            changes.lmtPrice = new Decimal(String(params.lmtPrice));
        if (params.auxPrice != null)
            changes.auxPrice = new Decimal(String(params.auxPrice));
        if (params.trailStopPrice != null)
            changes.trailStopPrice = new Decimal(String(params.trailStopPrice));
        if (params.trailingPercent != null)
            changes.trailingPercent = new Decimal(String(params.trailingPercent));
        if (params.orderType != null)
            changes.orderType = params.orderType;
        if (params.tif != null)
            changes.tif = params.tif;
        if (params.goodTillDate != null)
            changes.goodTillDate = params.goodTillDate;
        ...
        for (const { orderId, symbol, localSymbol } of candidates) {
            ...
            if (status !== 'Submitted' && status !== 'PreSubmitted') {
                ...
                const filledQty = orderFilledQty && !orderFilledQty.equals(UNSET_DECIMAL)
                    ...
                    : undefined;
                const currentStatus = status === 'Filled' ? 'filled' : status === 'Cancelled' ? 'cancelled' : 'rejected';
                if (currentStatus === 'filled' && (!filledQty || !brokerOrder.avgFillPrice)) {
                    ...
                    console.warn(`UTA[${this.id}]: order ${orderId} (${symbol}) synced to filled but broker omitted ` +
    ...
    _pollBackoffDue(orderId) {
        ...
        if (now - state.lastPolledAt < interval)
            return false;
    ...
    async getAccount() {
        ...
        const summable = positions.every((p) => (p.currency || account.baseCurrency) === account.baseCurrency);
        if (summable) {
     ...
     */
    async _reconcileWalletPositions(positions) {
        ...
        const inFlight = new Set(this.git.getPendingOrderIds().map((p) => p.aliceId).filter(Boolean));
        for (const p of walletPositions) {
            ...
            if (!final)
                continue; // Should be unreachable — reconcile would seed it.
     ...
     */
    _buildReconcileStateAfter(positions) {
        return {
            netLiquidation: '0',
            totalCashValue: '0',
            unrealizedPnL: '0',
     ...
     */
    async expandContract(aliceId, filters) {
    ...
    async searchContracts(pattern) {
        ...
        for (const desc of results)
            this.stampAliceId(desc.contract);
        return results;
    }
    /**
    ...
    async refreshCatalog() {
        if (typeof this.broker.refreshCatalog !== 'function')
            return;
    ...
    async getContractDetails(query) {
        ...
        if (details)
            this.stampAliceId(details.contract);
        return details;
    }
    ...
    _expandAliceIdIfNeeded(contract) {
        const hasNative = !!(contract.symbol || contract.localSymbol);
        if (!contract.aliceId || hasNative)
            return contract;
        ...
        for (const key of Object.keys(src)) {
            ...
            if (typeof value === 'number' && (value === 0 || value === UNSET_INTEGER || value === UNSET_DOUBLE))
                continue;
            dst[key] = value;
        }
        return expanded;
...
//# sourceMappingURL=UnifiedTradingAccount.js.map