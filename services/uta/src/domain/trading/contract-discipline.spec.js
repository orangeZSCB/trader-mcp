...
import { isSecType, validateContract, assertContract, SEC_TYPES, } from './contract-discipline.js';
function makeContract(overrides = {}) {
    ...
    if (overrides.lastTradeDateOrContractMonth)
        c.lastTradeDateOrContractMonth = overrides.lastTradeDateOrContractMonth;
    if (overrides.strike != null)
        c.strike = overrides.strike;
    if (overrides.right)
        c.right = overrides.right;
    if (overrides.multiplier)
        c.multiplier = overrides.multiplier;
    if (overrides.localSymbol)
        c.localSymbol = overrides.localSymbol;
    return c;
}
...
describe('validateContract — universal fields', () => {
    ...
    it('rejects empty symbol', () => {
        ...
        if (!r.ok)
            expect(r.errors.some(e => /symbol/.test(e))).toBe(true);
    });
    it('rejects unknown secType', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors.some(e => /secType .* not a known SecType/.test(e))).toBe(true);
    });
...
describe('validateContract — OPT/FOP requirements', () => {
    ...
    it('OPT with bad right value fails', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors.some(e => /right must be C\/P/.test(e))).toBe(true);
    });
});
...
//# sourceMappingURL=contract-discipline.spec.js.map