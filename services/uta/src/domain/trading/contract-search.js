...
export async function searchTradeableContracts(manager, pattern, assetClass = 'unknown') {
    ...
    const settled = await Promise.allSettled(targets.map(async (uta) => ({ id: uta.id, broker: uta.broker, results: await uta.searchContracts(brokerPattern) })));
