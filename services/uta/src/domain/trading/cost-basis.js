 ...
 */
export function recomputeCostBasisFromCommits(commits, aliceId) {
    ...
    const applyFill = (fillQty, fillPrice, direction) => {
        if (direction === 'buy') {
...
function matchesAliceId(op, aliceId) {
    switch (op.action) {
        case 'placeOrder':
        case 'closePosition':
...
export function isCostBasisRelevant(op, result) {
    if (!result?.success || !result.filledQty || !result.filledPrice)
        return false;
