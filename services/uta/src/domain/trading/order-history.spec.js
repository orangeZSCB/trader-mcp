...
let n = 0;
function commit(ops, results, message = 'test') {
    return {
        hash: `c${++n}`,
        parentHash: null,
        message,
...
describe('projectOrderHistory', () => {
    it('collapses place→sync-fill into one resolved row', () => {
        const commits = [
            commit([{ action: 'placeOrder', contract: contract(), order: limitBuy('0.01', '1650') }], [{ action: 'placeOrder', success: true, orderId: 'o1', status: 'submitted' }], 'buy the dip'),
            commit([{ action: 'syncOrders' }], [{ action: 'syncOrders', success: true, orderId: 'o1', status: 'filled', filledQty: '0.01', filledPrice: '1648.5' }]),
        ];
    ...
    it('external observed orders are flagged and resolvable by cancel', () => {
        const commits = [
            commit([{ action: 'observeExternalOrder', contract: contract(), order: limitBuy('1', '1500') }], [{ action: 'observeExternalOrder', success: true, orderId: 'ext1', status: 'submitted' }], '[observed] 1 external order(s)'),
            commit([{ action: 'cancelOrder', orderId: 'ext1' }], [{ action: 'cancelOrder', success: true, orderId: 'ext1', status: 'cancelled' }]),
        ];
        const rows = projectOrderHistory(commits);
        expect(rows).toHaveLength(1);
    ...
    it('preserves IBKR-superset option fields on the contract', () => {
        ...
        const commits = [
            commit([{ action: 'placeOrder', contract: opt, order: limitBuy('1', '12.5') }], [{ action: 'placeOrder', success: true, orderId: 'opt1', status: 'submitted' }]),
        ];
    ...
    it('rejected-before-submit rows survive without an orderId', () => {
        const commits = [
            commit([{ action: 'placeOrder', contract: contract(), order: limitBuy('0.01', '99999') }], [{ action: 'placeOrder', success: false, status: 'rejected', error: 'price band' }]),
        ];
        const rows = projectOrderHistory(commits);
        expect(rows).toHaveLength(1);
...
describe('projectTradeHistory', () => {
    it('records sync fills once with side/contract joined from the origin', () => {
        const commits = [
            commit([{ action: 'placeOrder', contract: contract(), order: limitBuy('0.01', '1650') }], [{ action: 'placeOrder', success: true, orderId: 'o1', status: 'submitted' }]),
            commit([{ action: 'syncOrders' }], [{ action: 'syncOrders', success: true, orderId: 'o1', status: 'filled', filledQty: '0.01', filledPrice: '1648.5' }]),
        ];
    ...
    it('reconcile foldings are labeled, not disguised as fills', () => {
        const commits = [
            commit([{ action: 'reconcileBalance', aliceId: 'okx|ETH/USDT', quantityDelta: '0.5', markPrice: '1700' }], [{ action: 'reconcileBalance', success: true, status: 'filled', filledQty: '0.5', filledPrice: '1700' }]),
        ];
        const trades = projectTradeHistory(commits);
        expect(trades).toHaveLength(1);
    ...
    it('does not double-count an origin fill against a redundant sync', () => {
        ...
        const commits = [
            commit([{ action: 'placeOrder', contract: contract(), order }], [{ action: 'placeOrder', success: true, orderId: 'o2', status: 'filled', filledQty: '1', filledPrice: '100' }]),
            commit([{ action: 'syncOrders' }], [{ action: 'syncOrders', success: true, orderId: 'o2', status: 'filled', filledQty: '1', filledPrice: '100' }]),
        ];
        expect(projectTradeHistory(commits)).toHaveLength(1);
    });
});
//# sourceMappingURL=order-history.spec.js.map