...
describe('TradingGit', () => {
    ...
    describe('sentinel boundary (OrderHelper.toWire)', () => {
        ...
        it('modifyOrder.changes also strips sentinels', () => {
            ...
            expect(op.changes.lmtPrice.toFixed()).toBe('150');
        ...
        it('non-sentinel Decimal fields survive (round-trip safety)', async () => {
            ...
            const op = exported.commits[0].operations[0];
            expect(op.order.lmtPrice).toBeInstanceOf(Decimal);
            expect(op.order.lmtPrice.toFixed()).toBe('3500.25');
        ...
        it('staging mutation after status() does not affect prior projection', () => {
            ...
            git.add(buyOp());
            const s1 = git.status();
            const op1 = s1.staged[0];
            op1.order.lmtPrice = 'tampered';
            const s2 = git.status();
            const op2 = s2.staged[0];
    ...
    describe('exportState / restore', () => {
        ...
        it('rehydrates legacy number-typed price fields to Decimal', async () => {
            ...
            const committedOp = raw.commits[0].operations[0];
            committedOp.order.lmtPrice = 145.25;
...
//# sourceMappingURL=TradingGit.spec.js.map