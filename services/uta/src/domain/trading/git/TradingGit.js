...
import { Order, UNSET_DECIMAL, UNSET_DOUBLE } from '@traderalice/ibkr';
...
function generateCommitHash(content) {
    const hash = createHash('sha256')
        .update(JSON.stringify(content))
        .digest('hex');
    return hash.slice(0, 8);
}
export class TradingGit {
    stagingArea = [];
    pendingMessage = null;
    pendingHash = null;
    commits = [];
    ...
    async recordObservedOrders(params) {
        ...
        const timestamp = new Date().toISOString();
        const operations = observed.map((o) => ({
            action: 'observeExternalOrder',
            contract: o.contract,
            order: o.order,
        }));
        const results = observed.map((o) => ({
            action: 'observeExternalOrder',
            success: true,
            orderId: o.orderId,
            status: 'submitted',
    ...
    getKnownOrderIds() {
        ...
        for (const commit of this.commits) {
            for (const result of commit.results) {
                if (result.orderId)
                    known.add(result.orderId);
                for (const leg of result.legs ?? [])
                    known.add(leg.orderId);
            }
        }
        return known;
    }
    ...
    log(options = {}) {
        ...
        if (symbol) {
            commits = commits.filter((c) => c.operations.some((op) => getOperationSymbol(op) === symbol));
        }
    ...
    buildOperationSummaries(commit, filterSymbol) {
        const summaries = [];
    ...
    formatOperationChange(op, result) {
        switch (op.action) {
            case 'placeOrder': {
    ...
    projectOperation(op) {
        if (op.action === 'placeOrder' || op.action === 'observeExternalOrder') {
            return { ...op, order: OrderHelper.toWire(op.order) };
        }
        if (op.action === 'modifyOrder') {
            return { ...op, changes: OrderHelper.toWire(op.changes) };
        }
        return op;
    }
    ...
    static rehydrateGitState(state) {
        return {
            ...state,
    ...
    // ==================== Sync ====================
    async sync(updates, currentState) {
        if (updates.length === 0) {
    ...
    getPendingOrderIds() {
        ...
        for (let i = this.commits.length - 1; i >= 0; i--) {
            for (const result of this.commits[i].results) {
                ...
                for (const leg of result.legs ?? []) {
                    if (!orderStatus.has(leg.orderId))
                        orderStatus.set(leg.orderId, 'submitted');
                }
            }
        }
        ...
        for (const commit of this.commits) {
            for (let j = 0; j < commit.results.length; j++) {
                ...
                const hasContract = op?.action === 'placeOrder' || op?.action === 'closePosition' || op?.action === 'observeExternalOrder';
                ...
                for (const orderId of candidates) {
                    if (seen.has(orderId) || orderStatus.get(orderId) !== 'submitted')
                        continue;
                    pending.push({
                        orderId,
                        symbol,
    ...
    // ==================== Simulation ====================
    async simulatePriceChange(priceChanges) {
        ...
        for (const { symbol, change } of priceChanges) {
            ...
            if (symbol === 'all') {
                ...
                for (let i = 0; i < positions.length; i++) {
                    ...
                    if ((pos.contract.symbol || pos.contract.aliceId) !== symbol)
                        continue;
        ...
        const worst = simulatedPositions.reduce((w, p) => (new Decimal(p.pnlChange).lt(w.pnlChange) ? { ...p, pnlChange: new Decimal(p.pnlChange) } : w), { ...simulatedPositions[0], pnlChange: new Decimal(simulatedPositions[0].pnlChange) });
        const excludedNote = excludedDerivatives.length > 0
            ...
            : '';
        const worstCase = (worst.pnlChange.lt(0)
    ...
    parseOperationResult(op, raw) {
        ...
        if (!success) {
            return {
                ...
                success: false,
                status: 'rejected',
                error: rawObj.error ?? 'Unknown error',
                raw,
            };
        }
        const orderId = rawObj.orderId;
        const orderState = rawObj.orderState;
        const legs = rawObj.legs;
        return {
            action: op.action,
            success: true,
            orderId,
...
//# sourceMappingURL=TradingGit.js.map