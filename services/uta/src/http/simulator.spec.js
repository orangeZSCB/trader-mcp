...
function makeCtx(brokers) {
    const utas = new Map();
    for (const [id, b] of Object.entries(brokers)) {
        utas.set(id, { id, label: id, broker: b });
    }
    return {
        utaManager: {
            get: (id) => utas.get(id),
...
async function req(routes, method, path, body) {
    const init = { method };
    if (body !== undefined) {
...
describe('POST /uta/:id/mark-price', () => {
    it('sets markPrice + auto-fills触达 limit orders', async () => {
        ...
        expect(status).toBe(200);
        expect(body.filled).toEqual([placed.orderId]);
...
//# sourceMappingURL=simulator.spec.js.map