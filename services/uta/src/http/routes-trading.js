...
const placeOrderSchema = z.object({
    ...
    message,
}).refine((d) => d.totalQuantity != null || d.cashQty != null, { message: 'Either totalQuantity or cashQty is required' });
...
async function runOneShot(c, uta, message, stage) {
 ...
 * - Permanent error → 500
 */
async function queryAccount(c, account, fn) {
...
export function createTradingRoutes(ctx) {
    const app = new Hono();
    // ==================== UTA listing ====================
    app.get('/uta', (c) => {
    ...
    app.get('/contracts/search', async (c) => {
        ...
        const rawAc = c.req.query('assetClass');
        const assetClass = rawAc && ALLOWED_ASSET_CLASSES.has(rawAc) ? rawAc : 'unknown';
    ...
    app.get('/fx-rates', async (c) => {
        ...
        for (const uta of ctx.utaManager.resolve()) {
            ...
            try {
                ...
                if (account.baseCurrency && account.baseCurrency !== 'USD')
                    currencies.add(account.baseCurrency);
            }
    ...
    app.post('/test-connection', async (c) => {
        ...
        try {
            ...
            const { utaConfigSchema } = await import('@openalice-trading/alice-core/core/config.js');
    ...
    app.post('/uta/:id/wallet/reject', async (c) => {
        ...
        if (!uta.status().pendingMessage)
            return c.json({ error: 'Nothing to reject' }, 400);
        try {
            const body = await c.req.json().catch(() => ({}));
    ...
    app.post('/uta/:id/wallet/push', async (c) => {
        ...
        if (!uta)
            return c.json({ error: 'Account not found' }, 404);
        if (!uta.status().pendingMessage)
            return c.json({ error: 'Nothing to push' }, 400);
        try {
    ...
    app.post('/uta/:id/wallet/place-order', async (c) => {
        const uta = ctx.utaManager.get(c.req.param('id'));
        if (!uta)
            return c.json({ error: 'UTA not found' }, 404);
        let body;
        try {
    ...
    app.delete('/uta/:id/snapshots/:timestamp', async (c) => {
        if (!ctx.snapshotService)
            return c.json({ error: 'Snapshot service not available' }, 503);
        const id = c.req.param('id');
    ...
    app.get('/snapshots/equity-curve', async (c) => {
        ...
        try {
            ...
            const perAccount = await Promise.all(accounts.map(async (uta) => {
...
//# sourceMappingURL=routes-trading.js.map