...
import { SEC_TYPES } from '../domain/trading/contract-discipline.js';
// ==================== Schemas ====================
...
const contractSchema = z.object({
    ...
    secType: z.enum([...SEC_TYPES]).optional(),
...
function resolveMock(ctx, c) {
    const id = c.req.param('id');
    if (!id)
        return { error: c.json({ error: 'Missing UTA id' }, 400) };
    const uta = ctx.utaManager.get(id);
    if (!uta)
        return { error: c.json({ error: `UTA ${id} not found` }, 404) };
...
async function parseBody(c, schema) {
    ...
    try {
        ...
        return { ok: false, error: c.json({ error: 'Invalid JSON' }, 400) };
    }
...
// ==================== Routes ====================
export function createSimulatorRoutes(ctx) {
    const app = new Hono();
...
//# sourceMappingURL=routes-simulator.js.map