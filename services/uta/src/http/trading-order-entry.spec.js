...
function makeMockUTA(opts = {}) {
    ...
    return {
        ...
        uta: {
            id: 'mock-uta',
            label: 'Mock UTA',
            stagePlaceOrder: vi.fn((p) => {
                calls.push({ method: 'stagePlaceOrder', args: [p] });
                if (opts.stageThrows === 'stage')
                    throw new Error('Invalid aliceId');
            }),
            stageClosePosition: vi.fn((p) => {
                calls.push({ method: 'stageClosePosition', args: [p] });
                if (opts.stageThrows === 'stage')
                    throw new Error('No such position');
            }),
            stageCancelOrder: vi.fn((p) => {
...
describe('POST /uta/:id/wallet/place-order', () => {
    ...
    it('preserves quantity precision through to staging (string, not float)', async () => {
        ...
        const stagedArg = mock.calls.find(c => c.method === 'stagePlaceOrder').args[0];
...
describe('POST /uta/:id/wallet/close-position', () => {
    ...
    it('happy path with explicit qty', async () => {
        ...
        expect(status).toBe(200);
        const stagedArg = mock.calls.find(c => c.method === 'stageClosePosition').args[0];
...
describe('POST /uta/:id/wallet/cancel-order', () => {
    ...
    it('happy path', async () => {
        ...
        expect(status).toBe(200);
        const stagedArg = mock.calls.find(c => c.method === 'stageCancelOrder').args[0];
...
//# sourceMappingURL=trading-order-entry.spec.js.map